...
- The Runtime 017 raw-AVRCP sidecar runs elevated **only** via the on-demand task `OpenDitoo Raw AVRCP Broker` from admin-only `C:\Program Files\OpenDitoo\RawAvrcpBroker`; any broker rebuild needs `runtime/windows/install_openditoo_raw_avrcp_task.ps1` re-run from Administrator PowerShell, and must never point the task at user-writable paths (`C:\BTP` is Authenticated Users:Modify).
