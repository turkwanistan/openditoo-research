...
**Current successor checkpoint (2026-09-10, end of session):** HF-3 is **PASS** (`OPENDITOO-INTERACTIVE-HF3-008`, on branch `feat/high-fps-interactive-pages`; 001–007 consumed, never re-arm). The accepted architecture is ONE `streaming_ack_clock` Host session carrying every page (`PageCarousel`, pixel-only paging), with a client floor + Host-anchored pacing and Host-confirmed yield handling. Never close/reopen the Host session to change pages. HF-4 is LIVE as Runtime 009 (`host/product_runtime_v3.py`, `product/OPENDITOO-PRODUCT-RUNTIME-009.json`; standing acceptance PASS 2026-09-11). Any change to its hashed modules needs a fresh revision, a branch, and an exact named grant.
...
- The product runs as a **systemd user service, which has no WSL-appended Windows `PATH`**: call every Windows executable by absolute `/mnt/c/...` path (Runtime 017's first cutover crash-looped on bare `schtasks.exe`, 2026-09-11). Rehearse new Windows-helper code inside `systemd-run --user --wait --pipe`, not an interactive shell.
- The raw-AVRCP sidecar runs elevated **only** via a per-runtime on-demand task (`OpenDitoo Raw AVRCP Broker 018`) from a per-runtime admin-only root (`C:\Program Files\OpenDitoo\RawAvrcpBroker018`); a successor gets its own root/task so the rollback runtime's pinned bytes stay intact. After any broker rebuild re-run `runtime/windows/install_openditoo_raw_avrcp_task.ps1 -Runtime NNN` from Administrator PowerShell; never point a task at user-writable paths (`C:\BTP` is Authenticated Users:Modify).
- BTVS is a GUI app: closing its window kills the capture. It must be started hidden (Runtime 018).
