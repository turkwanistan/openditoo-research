...
8. Run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/prepare_webcam_w8_telemetry_rerun_windows.ps1`. It is no-device prep only. After it passes, request `Grant OPENDITOO-WEBCAM-N980P-005`; then execute only `scripts/run_webcam_w8_telemetry_rerun_windows.ps1`. W9 remains optional; W10 remains product polish/separate webcam-authority decision.
