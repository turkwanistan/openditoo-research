...
| W8 near-ceiling ACK-clock trial | **003 consumed/unknown; 004 awaiting Windows prep** — 003 hit `SESSION_PACING_VIOLATION` after 2 ACKed frames because equal 40 ms client/Host floors leave no request-arrival margin; 004 uses 50 ms client / 40 ms Host and 201/603/211,854 ceilings |
...
4. **W8 attempt 003 is consumed and must never be replayed.** It ACKed 2 frames then the third frame POST hit `SESSION_PACING_VIOLATION`; client/claim outcome remains `unknown`, while the Host ledger independently shows its session terminal as `stopped_clean` after refusing the early frame. Runtime 003 was restored and verified connected.
5. Root cause is measured, not inferred from ACK speed alone: 003's client dispatch intervals were just over 40 ms, but the Host enforces 40 ms between **Host-side frame starts**. Variable outbound localhost/serialization latency can shrink Host-arrival spacing below the client's spacing. Equal 40/40 floors are unsafe.
6. **Fresh replacement 004 uses 50 ms client / 40 ms Host**, still ACK-gated, one frame in flight, zero packet spacing, no retry/reconnect/reclaim/pipelining. The 10 ms margin leaves a 20 fps theoretical client ceiling; 10 s budgets are 201 frames / 603 packets / 211,854 bytes. A new offline control must reproduce the 40 ms failure under 8 ms arrival skew and pass 50 ms under the same skew.
7. Run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/prepare_webcam_w8_rerun_windows.ps1`. It is no-device prep only. After it passes, request the distinct grant `Grant OPENDITOO-WEBCAM-N980P-004`; then execute only `scripts/run_webcam_w8_rerun_windows.ps1`. W9 remains optional; W10 remains product polish/separate webcam-authority decision.
