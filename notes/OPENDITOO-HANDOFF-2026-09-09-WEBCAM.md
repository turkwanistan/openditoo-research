...
| W8 near-ceiling ACK-clock trial | **PASS / consumed**: 005 ran its full 10 s lifetime — 163 frames / 489 packets / 170,737 bytes, 16.285 fps (quarters 15.6/16.4/16.4/16.8), source age at send p95 51.86 ms, `lifetime_expired` / `stopped_clean`, client and Host ledger in exact agreement, no retry/reconnect/reclaim, 0 duplicate source frames. owner visual PASS ("looked great", no tearing). **W8 closed PASS.** 003 (pacing) and 004 (telemetry validator) remain consumed failures |
| W9A source identity + motion truth | **implemented offline** (route §27): monotonic `SourceId` assigned at acquisition and propagated raw→transform→ready→selection; bounded `sourceIdentity` evidence (first/last/skipped/duplicates/out-of-order/512-id ring); `ADAPTER_W9A_SOURCE_IDENTITY_PASS`; deterministic 4×4 motion-truth stimulus round-trips all 4096 counters through the real transform. Transport unchanged; no manifest, claim, grant or device I/O |
| W9B unique-frame + optical latency | **attempt 006 consumed/unknown; no optical evidence yet** (route §29). Offline tooling and operator framing are verified, but the live trial failed: Host got no ACK within the 5000 ms budget, `transport_fault` / `IMAGE_RX_RECV_TIMEOUT; NO_RETRY`, ledger 0/0/0, nothing displayed. Client side was clean and W9A identity populated correctly (source id 76, 0 duplicates, 0 out-of-order), so the instrumentation is exonerated. Device answered Runtime 003 ~0.24 s later. Root cause LOW confidence. A further trial needs a fresh identity and a new exact grant |
...
8. **W8-005 is PASS and consumed** (route §26). One bounded run after the exact grant:
   ...
   Owner visual observation is **PASS** — "looked great", no tearing or artifacts reported versus the
   W7 ~10.78 fps baseline; that accepts visible usability at the higher cadence only, not panel refresh.
   **W8 is closed PASS.** Attempt 005 is consumed and replay-forbidden; W9A is next.
