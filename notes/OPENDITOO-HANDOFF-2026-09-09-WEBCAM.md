...
| W9B unique-frame + optical latency | **offline tooling implemented** (route §28): `host/w9b_optical.py` + `cli/w9b.py` decode monitor (unmirrored) and Ditoo (mirrored) from one shared exposure, so no clock alignment is needed; first-appearance correlation gives p50/p95/min/max scene→visible latency plus visible transitions, repeats, skips and undecodable counts. Verified end to end on synthetic footage: injected 7-frame lag at 240 fps recovered as 29.1667 ms. Operator slow-mo confirmed available; physical setup, a fresh manifest and a new exact grant still required |
