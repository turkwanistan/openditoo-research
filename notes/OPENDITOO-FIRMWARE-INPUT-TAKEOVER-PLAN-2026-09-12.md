...
**Recovery fallback: MassBoot (Route B), de-prioritized for retail USB.** Static firmware proves a Lighting+external-power MassBoot selector and the exact unit changes boot behavior under that condition. Two enumeration-only trials, including a replacement-cable repeat, produced no host-visible Windows USB/PnP delta. Do not spend more time on external cable/port swaps unless new hardware evidence changes the model.
