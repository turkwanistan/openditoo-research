...
A provenance-strong **Divoom 3.1.58** Android APK from 2021-11-17 is now preserved as `artifacts/apps/divoom_android_3.1.58.apk`. Its SHA-1 `ac5158aad4a88f56772c5180887697d5c5b9415c` exactly matches the historical 3.1.58 SHA-1 already recorded from REvoom metadata. Its package/version witnesses are `com.divoom.Divoom` / `3.1.58`; ZIP integrity is clean; and its Android signing certificate exactly matches the independent APKMirror Divoom signer oracle (SHA-1 `eeb353cb9bc057f8abd82cfc36f483d745ff9dd0`, SHA-256 `b54bda23c5c9239bb80a0581220f2d46c3f59962c3ec52e692202e65b5ac0955`). Full provenance is `artifacts/provenance/divoom_android_3.1.58.json`.

Offline decompilation found two genuine hidden developer controls in `AboutTestFragment`: a **test/formal upgrade-version switch** backed by `com.divoom.DIVOOM.SP_TEST_VERSION`, and a separate **test/formal server switch** backed by `com.divoom.DIVOOM.SP_TEST_SERVER`. The first sets the firmware API `IsTest` boolean; the second changes the app API host between `app.divoom-gz.com` and `apptest.divoom-gz.com`.

The period firmware request itself is narrow: `GetUpdateFileRequest` carries `Hardware`, `IsTest`, `Language`, and `UpdateFlag`, with inherited `DeviceId`; its constructor fixes `UpdateFlag=2`, and no `setUpdateFlag()` caller exists in the recovered client. There is exactly one `GetUpdateFileV3` construction/call site. `GetUpdateFileResponse` contains only `FileId`, `Version`, `Sha1`, and `Explain`. Returned FileIds are passed through the ordinary Divoom firmware-CDN helper, SHA-1 checked, and handed to the existing update-push path. No `divoomupdate` filename string is present in the APK.

A bounded four-request check of the still-live period test host (`Hardware=42/60 × IsTest=false/true`, V3 only, no parameter/product/update-flag enumeration) is preserved at `artifacts/provenance/period_apptest_v3_matrix_2026-09-12.json`. It currently resolves to the **same four firmware objects** as the modern V3 service: flag42 42016/42017 and flag60 60014/60016. This does not reconstruct what the test host served historically, but it closes the current legacy-host lead without a retry ladder.

The machine-checkable offline summary is `artifacts/analysis/period_app_recovery_surface.json`, generated by `tools/ditoo_period_app_recovery.py`.
...
### SFR-2 — period app acquisition and updater-diff archaeology — CLOSED (2026-09-12)

**Result:** `PERIOD_APP_NO_HISTORICAL_OR_RECOVERY_SELECTOR_FOUND`.

The provenance-strong 3.1.58 client was recovered, identity/signer-pinned, decompiled offline, and its current legacy test backend checked with the exact bounded client schema. It exposes real developer **test branch** and **test server** toggles, but no arbitrary/historical firmware-version selector, no app-side `UpdateFlag` override, and no distinct support/SD-recovery path. The legacy test backend currently returns the same production/test firmware objects as the modern service and does not yield v42012.

This closes the planned period-app discriminator, not the historical distribution question. One recovered 2021 client cannot prove every older app/support backend lacked other behavior, and failure to recover `v42012` or `divoomupdate.bin` is not proof those bytes were never distributed. Future provenance-trackable historical artifacts can still be ingested opportunistically without reopening software-first work as an unbounded search.
...
Software-first work was continued through every planned discriminating branch. Do not declare exact v42012 externally recoverable merely because nearby firmware exists. **SFR-1, SFR-2, SFR-3 and the first SFR-4 lineage expansion are now closed**, and no provenance-trackable v42012 image emerged. The software-only route has therefore reached the evidence-backed blocker defined below. Opportunistic future v42012/support-package acquisition remains welcome, but is no longer an active unbounded search.

If all of the following become true:

1. SFR-1 remains closed without a reachable stock readback chain after the new lineage checks — **met**;
2. period app/support-updater archaeology yields no historical-v42012 source or recovery endpoint — **met by SFR-2**;
3. no provenance-trackable v42012 image emerges — **met as of this checkpoint**;

The software-only route has therefore reached a meaningful evidence-backed blocker. At that point, present the owner with the choice rather than silently escalating: keep the project recovery-gated, or return to the already-prepared **photos-only M1** disassembly session to determine whether MassBoot/SPI offers a simple no-solder route.
...
- `python3 tools/ditoo_period_app_recovery.py --selfcheck` — **PASS**;
- `SoftwareRecoverySurfaceTests` + `OfficialTestBranchLineageTests` + `PeriodAppRecoverySurfaceTests` — **16/16 PASS**;
- `python3 scripts/verify_day1_offline.py` — **361 tests**, exactly **2 errors**, both the pre-existing unrelated W9B optical tests caused by missing `PIL`; **zero software-recovery / firmware-R&D test failures**;
- `git diff --check` — **PASS**;
