...
008 physically accepted all production status colors/transitions. The next authority shape is intentionally separate from N2–N5 one-use experiments: `product/OPENDITOO-PRODUCT-RUNTIME-001.json` is a committed **disabled** template whose code hashes freeze the supervisor, renderer, collector and bounded-session source. A future explicit product grant materializes only a local mode-0600 policy. The supervisor reuses the accepted Host session endpoints; no raw-send/target route was added. It renews bounded sessions, immediately opens a fresh session after `canvas_invalidated`, backs off 1/2/5/10/30 s after genuine Host/device failure, and starts from a separate Windows logon bootstrap plus WSL systemd user service. The standalone collector is transactionally paused to avoid cursor writers racing and restored exactly on rollback/uninstall. Nothing is currently authorized.
