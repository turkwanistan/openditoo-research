...
The first Runtime 011 cutover attempt on candidate `c7469a6` reached the main-checkout offline gate and **auto-rolled back before local 011 authority was written or the service restarted**. The only failures were three stale broad-gate tests that still treated Runtime 010 as the current hash-valid template. Live Runtime 010 was restored exactly. The repaired successor is now on `feat/pocket-moss-v2-art-r2`; the 36 Runtime 011 code/asset hashes, Host hash, ButtonProbe hashes and requested grant scope are unchanged from `c7469a6`, so the already-given exact Runtime 011 grant remains in-scope.

A visual-only successor is prepared offline as **Runtime 011** (`runtime_revision=6`) on `feat/pocket-moss-v2-art-r2`. It is intentionally unauthorized. No Runtime 011 device I/O has occurred.
