...
The client fix keeps the Host's hard 150 ms floor but uses a nominal **200 ms MCP send cadence** with a phase-stable **50 ms render tick**. This leaves 50 ms timing headroom and produces a ~0.8 s four-stage pulse. Offline verification passes 152 tests. Fresh manifest: `experiments/DAY1-M9-ACTIVATION-004.json`. **`OPENDITOO-M9-ACTIVATION-004` is currently authorized for ONE bounded execution only** under the operator grant `Grant OPENDITOO-M9-ACTIVATION-004`. Streaming remains tabled until MCP dashboard acceptance is complete.
