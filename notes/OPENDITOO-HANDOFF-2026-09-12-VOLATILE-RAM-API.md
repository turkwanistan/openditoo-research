...
**Control sink / trigger:** runtime50 is a 0x50-byte object whose `+0x34` field is a genuine function pointer. The `0x1fa8a` family loads `object+0x34`, checks it for zero, and `BLX`es it; the `0x1fd6a` family explicitly stores an incoming callback value into that field. This survives 4/4 preserved Plus branches. VoiceTip worker registration is now also **proven 4/4**, not framework-opaque: the branch-local init path calls `Fwl_MicroTask` with argument 0 and period/reload 1; v42016/flag60 register callback `0x00218b43`, while v42017 registers `0x00218b37`. The callback namespace obeys `0x00200000 + (worker_file_offset - 0x7000) + ThumbBit`, decoding exactly to `0x1fb42` or the v42017-relocated `0x1fb36`. The branch-local `Fwl_MicroTask` veneer converges on the same 8-slot/12-byte registration implementation, and resident timer code at the `0x14f8/0x15ec` family eventually loads `slot.callback`/`slot.argument` and `BLX`es it. The periodic worker reaches `0x1fa8a` on normal VoiceTip completion/idle paths, but not on every tick. Direct stock `0x6c` and `0xa9` handler bodies still do not guarantee that completion state; `0xa9` primarily drives a separate recorded-voice state machine and shared teardown does not itself call `+0x34`. The trigger blocker is now `STOCK_VOICETIP_COMPLETION_STATE_TO_RUNTIME50_CALLBACK_UNPROVEN`.
