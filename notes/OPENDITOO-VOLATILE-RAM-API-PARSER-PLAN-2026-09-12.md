...
5. independently resolve invocation: VoiceTip worker registration is now proven through the stock period-1 `Fwl_MicroTask` path and resident dispatcher. Find a deterministic ordinary stock state transition that makes the next/near-term periodic worker tick take its completion/idle path into `0x1fa8a`. Direct `0x6c` does not do this, and `0xa9` primarily controls a separate recorded-voice path; blocker = `STOCK_VOICETIP_COMPLETION_STATE_TO_RUNTIME50_CALLBACK_UNPROVEN`;
