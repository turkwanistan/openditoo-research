#!/usr/bin/env python3
"""Narrow OpenDitoo Day-1 CLI.

Current surface is intentionally offline/status-only. It cannot discover, pair,
connect, send Bluetooth bytes, or select a device target.
"""
...
def host_status(_: argparse.Namespace) -> int:
    ...
    if not isinstance(body, dict) or any(body.get(k) != v for k, v in required.items()):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="openditoo", description="OpenDitoo Day-1 offline/status CLI")
    sub = parser.add_subparsers(dest="command", required=True)

    p = sub.add_parser("auth-init", help="create a separate OpenDitoo local Host token")
    p.set_defaults(func=auth_init)
    p = sub.add_parser("status", help="query only the separate status-only OpenDitoo Host")
