...
## 2026-09-12 volatile-RAM API pivot — VRAM-8A CLOSED OFFLINE / VRAM-8B OWNER-GRANT BOUNDARY — HISTORICAL SNAPSHOT

The active research route is `notes/OPENDITOO-HANDOFF-2026-09-12-VOLATILE-RAM-API.md` / `notes/OPENDITOO-VOLATILE-RAM-API-PARSER-PLAN-2026-09-12.md`, with the post-execution roadmap in `notes/OPENDITOO-VRAM8-11-RAM-API-ROADMAP-2026-09-12.md`. VRAM-0..7 remain CLOSED and non-replayable. **VRAM-8A is now CLOSED OFFLINE** across all four preserved Plus branches: the controlled callback ABI is pinned, and a deterministic 20-byte leaf at fresh Thumb entry `0x00804901` (`10b5034c6468e27b01235a40e27310bdf0308000`, SHA-256 `1cdd53f83a75021b47cedc65bbac9f5ee1d488863d029d5900d4687efcf5264b`) preserves callee-saved state, performs no calls/MMIO/flash/cache operation, toggles only the one-byte volatile energy-control field `*(u32*)0x008030f4 + 0x0f`, and returns normally. Stock `0xb2/0xb3` provide a fixed `0 -> 1 -> 0` typed baseline/positive-readback/restoration discriminator. The fresh entry intentionally avoids the already-executed VRAM67 `0x00804779` I-cache line. Reproducible authority is `tools/ditoo_vram8_stage0_model.py` + `artifacts/analysis/volatile_ram_api_vram8_stage0.{json,bin}`.

`OPENDITOO-VRAM8B-CANARY-001` is now **prepared offline but unauthorized**. Its manifest/fixture report freeze eight application sends on one connection, exactly one custom `0x6c`, no retry/reconnect, the 75-second callback hold, typed `0 -> 1 -> 0` stock readback, and Runtime 018 restoration as mandatory success conditions. No runner, grant, claim, Bluetooth session, Runtime 018 stop or packet transmission was created/performed during preparation. Existing VRAM67 grants do not transfer. **Next live boundary:** only the exact fresh 8B owner grant may authorize a future one-use canary run; 8D resident installation and all VRAM-9 work remain later independent gates.

VRAM-8C is **DESIGN_ADVANCED_NOT_CLOSED**. A single deliberately retained device-side app-heap allocation is the preferred resident window if/when callback context and allocator ABI/reentrancy are proven; hardcoded free-looking heap addresses, the display backing, runtime50/adjacent live objects and unreserved upper SRAM are rejected. The future installer remains fixed-destination/fixed-entry/bounded/integrity-checked, prefers one fixed resident image, requires explicit I-cache maintenance before stage-1 execution, and exposes no host-selected address/call target, `poke`, `goto`, generic ARM uploader, interpreter or raw-packet product surface. Runtime 018 remains the accepted everyday product. M2 unpowered mapping remains authorized/unconsumed but deferred.

Checkpoint verification: both VRAM-8 selfchecks PASS; focused trigger/VRAM3/8A/8B tests 17/17 PASS; `git diff --check` PASS; the single broad verifier run executed 390 tests with exactly the two pre-existing unrelated missing-Pillow W9B optical errors and no new VRAM/parser/product failure.
