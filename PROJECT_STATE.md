...
The active research route is `notes/OPENDITOO-HANDOFF-2026-09-12-VOLATILE-RAM-API.md` / `notes/OPENDITOO-VOLATILE-RAM-API-PARSER-PLAN-2026-09-12.md`, with the post-execution roadmap in `notes/OPENDITOO-VRAM8-11-RAM-API-ROADMAP-2026-09-12.md`. VRAM-0..7 are now CLOSED: deterministic placement/trigger/executable-heap evidence is promoted offline and consumed `OPENDITOO-VRAM67-BXLR-002` is the exact-unit LIVE PASS for the minimal returning `BX LR` witness under that composite model. The active objective is **VRAM-8A offline callback-ABI + positive nontrivial returning canary proof**, followed by a separate fresh-grant 8B live canary; resident-window/installer proof is 8C/8D, then API ingress/liveness/input claim is 9A/9B/9C. Runtime 018 remains the accepted everyday product. VRAM67-001/002 are non-replayable and grant no forward authority. M2 unpowered mapping remains authorized/unconsumed but deferred. Every VRAM-8+ live experiment still needs a fresh reviewed one-use manifest and exact owner grant.
