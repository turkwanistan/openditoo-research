...
internal static class WindowsRfcommModelSentinel009Transport
{
    ...
    internal static Result RunExactlyOnce(byte[][] fixtures)
    {
        ...
        try
        {
            ...
            if (startup != 0) throw new InvalidOperationException($"MODEL009_WSASTARTUP_FAILED WSA={startup}; NO_RETRY");
            started = true;
            var layoutSize = Marshal.SizeOf<SockAddrBth>();
            if (layoutSize != 30) throw new InvalidOperationException($"MODEL009_SOCKADDR_BTH_LAYOUT_REJECTED SIZE={layoutSize}");
            socketHandle = socket(AfBth, SockStream, BthProtoRfcomm);
            if (socketHandle == InvalidSocket) throw new InvalidOperationException($"MODEL009_SOCKET_CREATE_FAILED WSA={WSAGetLastError()}; NO_RETRY");
            eventHandle = WSACreateEvent();
            if (eventHandle == InvalidEvent) throw new InvalidOperationException($"MODEL009_EVENT_CREATE_FAILED WSA={WSAGetLastError()}; NO_RETRY");

            SelectEvents(socketHandle, eventHandle, FdConnect, "CONNECT");
            var remote = new SockAddrBth { AddressFamily = (ushort)AfBth, BluetoothAddress = FrozenModelSentinel009Protocol.TargetBluetoothAddress, ServiceClassId = Guid.Empty, Port = FrozenModelSentinel009Protocol.TargetRfcommChannel };
            var connectDeadline = new Deadline(FrozenModelSentinel009Protocol.ConnectBudgetMs);
            connectionsAttempted = 1;
            var cr = connect(socketHandle, ref remote, layoutSize);
            if (cr == SocketError)
            {
                var error = WSAGetLastError();
                if (error != WsaWouldBlock) throw new InvalidOperationException($"MODEL009_CONNECT_FAILED WSA={error}; NO_RETRY");
            ...
            sends++;
            Thread.Sleep(FrozenModelSentinel009Protocol.InterPacketDelayMs);
            ...
            if (sends != FrozenModelSentinel009Protocol.ExactApplicationSends) throw new InvalidOperationException($"MODEL009_SEND_COUNT_DRIFT sends={sends}; NO_RETRY");
            if (customOverwriteSends != 1) throw new InvalidOperationException($"MODEL009_CUSTOM_SEND_COUNT_DRIFT count={customOverwriteSends}; NO_RETRY");
            if (total.ElapsedMilliseconds > FrozenModelSentinel009Protocol.TotalBudgetMs) throw new TimeoutException("MODEL009_TOTAL_BUDGET_EXCEEDED; NO_RETRY");
    ...
    private static long CaptureWithScheduledOverwrite(IntPtr socketHandle, IntPtr eventHandle, Stopwatch observation, List<Report> reports, byte[] overwriteFixture, ref int sends, ref int customOverwriteSends)
    {
        ...
        while (true)
        {
            ...
            if (!overwriteSent && elapsed >= FrozenModelSentinel009Protocol.OverwriteTargetElapsedMs)
            {
                ...
                Console.WriteLine($"MODEL009_OVERWRITE_SENT elapsed_ms={overwriteElapsedMs} source_length=0x{FrozenModelSentinel009Protocol.CustomSourceLength:X}");
                continue;
            }

            if (elapsed >= FrozenModelSentinel009Protocol.CaptureWindowMs)
            {
                if (!overwriteSent) throw new InvalidOperationException("MODEL009_CAPTURE_ENDED_BEFORE_OVERWRITE; NO_RETRY");
                if (pending.Count != 0) throw new InvalidOperationException($"MODEL009_CAPTURE_ENDED_WITH_PARTIAL_FRAME bytes={pending.Count}; NO_RETRY");
                return overwriteElapsedMs;
            }

            var nextDeadlineMs = overwriteSent ? FrozenModelSentinel009Protocol.CaptureWindowMs : Math.Min(FrozenModelSentinel009Protocol.OverwriteTargetElapsedMs, FrozenModelSentinel009Protocol.CaptureWindowMs);
            ...
            if (wait == WsaWaitFailed) throw new InvalidOperationException($"MODEL009_CAPTURE_WAIT_FAILED WSA={WSAGetLastError()}; NO_RETRY");
            if (wait != WsaWaitEvent0) throw new InvalidOperationException($"MODEL009_CAPTURE_WAIT_UNEXPECTED RESULT={wait}; NO_RETRY");
            var events = WsaNetworkEvents.Create();
            if (WSAEnumNetworkEvents(socketHandle, eventHandle, ref events) == SocketError) throw new InvalidOperationException($"MODEL009_CAPTURE_ENUM_FAILED WSA={WSAGetLastError()}; NO_RETRY");
            if ((events.NetworkEvents & FdClose) != 0) throw new InvalidOperationException($"MODEL009_DISCONNECTED_DURING_CAPTURE WSA={events.ErrorCodes[FdCloseBit]}; NO_RETRY");
            if ((events.NetworkEvents & FdRead) == 0) continue;
            if (events.ErrorCodes[FdReadBit] != 0) throw new InvalidOperationException($"MODEL009_CAPTURE_READ_EVENT_FAILED WSA={events.ErrorCodes[FdReadBit]}; NO_RETRY");
            while (true)
            {
                var got = recv(socketHandle, chunk, chunk.Length, 0);
                if (got == 0) throw new InvalidOperationException("MODEL009_CAPTURE_RECV_DISCONNECTED; NO_RETRY");
                if (got < 0)
                {
                    var error = WSAGetLastError();
                    if (error == WsaWouldBlock) break;
                    throw new InvalidOperationException($"MODEL009_CAPTURE_RECV_FAILED WSA={error}; NO_RETRY");
                }
                for (var i = 0; i < got; i++) pending.Add(chunk[i]);
                if (pending.Count > FrozenModelSentinel009Protocol.MaxBufferedBytes) throw new InvalidOperationException($"MODEL009_CAPTURE_BUFFER_EXCEEDED bytes={pending.Count}; NO_RETRY");
                ParseAvailableFrames(pending, observation, reports, overwriteSent ? "post_overwrite" : "pre_overwrite");
            }
        }
    }
    ...
    private static void ParseAvailableFrames(List<byte> pending, Stopwatch observation, List<Report> reports, string phase)
    {
        while (true)
        {
            ...
            if (pending[0] != 0x01) throw new InvalidOperationException($"MODEL009_FRAME_START_REJECTED value=0x{pending[0]:X2}; NO_RETRY");
            if (pending.Count < 3) return;
            var innerLength = pending[1] | (pending[2] << 8);
            var expectedTotal = innerLength + 4;
            if (expectedTotal < 9 || expectedTotal > FrozenModelSentinel009Protocol.MaxWireBytes) throw new InvalidOperationException($"MODEL009_FRAME_LENGTH_REJECTED inner={innerLength} total={expectedTotal}; NO_RETRY");
            ...
            reports.Add(report);
            if (reports.Count > FrozenModelSentinel009Protocol.MaxReports) throw new InvalidOperationException($"MODEL009_REPORT_BUDGET_EXHAUSTED count={reports.Count}; NO_RETRY");
            Console.WriteLine($"MODEL009_REPORT elapsed_ms={report.ElapsedMs} phase={report.Phase} inner=0x{report.InnerCommand:X2} payload={report.PayloadHex} wire={report.WireHex}");
        }
    }

    private static Report ValidateAndDecode(byte[] wire, long elapsedMs, string phase)
    {
        if (wire[0] != 0x01 || wire[^1] != 0x02) throw new InvalidOperationException("MODEL009_RESPONSE_BOUNDARY_REJECTED; NO_RETRY");
        var innerLength = wire[1] | (wire[2] << 8);
        if (wire.Length != innerLength + 4 || innerLength < 5) throw new InvalidOperationException($"MODEL009_RESPONSE_INNER_LENGTH_REJECTED value={innerLength}; NO_RETRY");
        ...
        if (sum != observedChecksum) throw new InvalidOperationException($"MODEL009_RESPONSE_CHECKSUM_REJECTED observed=0x{observedChecksum:X4} expected=0x{sum:X4}; NO_RETRY");
        if (wire[3] != 0x04 || wire[5] != 0x55) throw new InvalidOperationException($"MODEL009_RESPONSE_WRAPPER_REJECTED outer=0x{wire[3]:X2} tag=0x{wire[5]:X2}; NO_RETRY");
    ...
    private static void WaitForInitialWritable(IntPtr socketHandle, IntPtr eventHandle, Deadline deadline)
    {
        while (true)
        {
            ...
            if ((events.NetworkEvents & FdClose) != 0) throw new InvalidOperationException($"MODEL009_INITIAL_WRITE_WINDOW_CLOSED WSA={events.ErrorCodes[FdCloseBit]}; NO_RETRY");
            if ((events.NetworkEvents & FdWrite) == 0) continue;
            if (events.ErrorCodes[FdWriteBit] != 0) throw new InvalidOperationException($"MODEL009_INITIAL_WRITE_WINDOW_FAILED WSA={events.ErrorCodes[FdWriteBit]}; NO_RETRY");
            return;
        }
    }
    ...
    private static void SendSingleCall(IntPtr socketHandle, IntPtr eventHandle, byte[] bytes, string stage)
    {
        ...
        if (sent != bytes.Length)
        {
            ...
            throw new InvalidOperationException($"MODEL009_{stage}_SEND_{suffix} bytes={sent}/{bytes.Length} WSA={error}; NO_RETRY");
        }
    }
    ...
    private static void CheckNoCloseEvent(IntPtr socketHandle, IntPtr eventHandle, string stage)
    {
        ...
        if (wait == WsaWaitFailed) throw new InvalidOperationException($"MODEL009_{stage}_PRE_SEND_WAIT_FAILED WSA={WSAGetLastError()}; NO_RETRY");
        if (wait != WsaWaitEvent0) throw new InvalidOperationException($"MODEL009_{stage}_PRE_SEND_WAIT_UNEXPECTED result={wait}; NO_RETRY");
        var events = WsaNetworkEvents.Create();
        if (WSAEnumNetworkEvents(socketHandle, eventHandle, ref events) == SocketError) throw new InvalidOperationException($"MODEL009_{stage}_PRE_SEND_ENUM_FAILED WSA={WSAGetLastError()}; NO_RETRY");
        if ((events.NetworkEvents & FdClose) != 0) throw new InvalidOperationException($"MODEL009_{stage}_CLOSED WSA={events.ErrorCodes[FdCloseBit]}; NO_RETRY");
    }

    private static void SelectEvents(IntPtr socketHandle, IntPtr eventHandle, int mask, string stage)
    {
        if (WSAEventSelect(socketHandle, eventHandle, mask) == SocketError) throw new InvalidOperationException($"MODEL009_{stage}_EVENT_SELECT_FAILED WSA={WSAGetLastError()}; NO_RETRY");
    }
    ...
    private static void WaitForExactEvent(IntPtr socketHandle, IntPtr eventHandle, int eventMask, int errorIndex, Deadline deadline, string stage)
    {
        while (true)
        {
            ...
            if (error != 0) throw new InvalidOperationException($"MODEL009_{stage}_FAILED WSA={error}; NO_RETRY");
            return;
        }
    }
    ...
    private static WsaNetworkEvents WaitForEvents(IntPtr socketHandle, IntPtr eventHandle, Deadline deadline, string stage)
    {
        ...
        if (wait == WsaWaitTimeout) throw new TimeoutException($"MODEL009_{stage}_TIMEOUT; NO_RETRY");
        if (wait == WsaWaitFailed) throw new InvalidOperationException($"MODEL009_{stage}_WAIT_FAILED WSA={WSAGetLastError()}; NO_RETRY");
        if (wait != WsaWaitEvent0) throw new InvalidOperationException($"MODEL009_{stage}_WAIT_UNEXPECTED result={wait}; NO_RETRY");
        var events = WsaNetworkEvents.Create();
        if (WSAEnumNetworkEvents(socketHandle, eventHandle, ref events) == SocketError) throw new InvalidOperationException($"MODEL009_{stage}_ENUM_FAILED WSA={WSAGetLastError()}; NO_RETRY");
        return events;
    }
    ...
    private sealed class Deadline(int budgetMs)
    {
        ...
        internal int RemainingMilliseconds(string stage)
        {
            ...
            if (remaining <= 0) throw new TimeoutException($"MODEL009_{stage}_TIMEOUT budget_ms={budgetMs}; NO_RETRY");
            return remaining;
        }
    }
}
