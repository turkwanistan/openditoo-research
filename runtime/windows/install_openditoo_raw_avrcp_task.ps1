...
# One-time elevated install of the Runtime 017 raw-AVRCP sidecar as an on-demand, highest-privilege scheduled task.
...
$policy = Get-Content -LiteralPath (Join-Path $Repository 'product\OPENDITOO-PRODUCT-RUNTIME-017.json') -Raw -Encoding UTF8 | ConvertFrom-Json
...
$root = 'C:\Program Files\OpenDitoo\RawAvrcpBroker'
Need ($task -eq 'OpenDitoo Raw AVRCP Broker' -and $launch.kind -eq 'scheduled_task_highest_on_demand') 'policy launch mismatch'

if ($Uninstall) {
    Stop-ScheduledTask -TaskName $task -ErrorAction SilentlyContinue
    Unregister-ScheduledTask -TaskName $task -Confirm:$false -ErrorAction SilentlyContinue
    if (Test-Path -LiteralPath $root) { Remove-Item -LiteralPath $root -Recurse -Force }
    Step 'R017_TASK_UNINSTALLED' 'PASS'
    exit 0
}
...
Step 'R017_TASK_SOURCES_VERIFIED' "$($pins.Count)"
...
Step 'R017_TASK_INSTALL_ROOT_ADMIN_ONLY' 'PASS'
...
Step 'R017_TASK_INSTALLED_SELFTEST' 'PASS'
...
Register-ScheduledTask -TaskName $task -Action $action -Principal $principal -Settings $settings -Force `
    -Description 'OpenDitoo Runtime 017 receive-only raw AVRCP sidecar (BTVS needs elevation). On demand only; exits when the WSL lease stops.' | Out-Null
Step 'R017_TASK_REGISTERED' "$task as $user"
Step 'R017_TASK_INSTALL' 'PASS'
