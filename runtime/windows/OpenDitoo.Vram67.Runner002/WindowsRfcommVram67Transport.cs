...
internal static class WindowsRfcommVram67Transport
{
    ...
    internal sealed record Result(int ConnectionsAttempted, int SendsCompleted, int CustomOverwriteSends, long ObservationMilliseconds, bool ConnectionSurvivedObservation);
    ...
    internal static Result RunExactlyOnce()
    {
        ...
        try
        {
            ...
            // Winsock FD_WRITE is not a per-send readiness clock. It is recorded when
            // the socket first becomes writable and, after that, when a send returned
            // WSAEWOULDBLOCK and buffer space becomes available again. Wait for the
            // initial writable state exactly once, then issue each frozen application
            // frame with exactly one send() call. WSAEWOULDBLOCK/short/error is a
            // terminal no-retry outcome for this one-use experiment.
            ...
            SendSingleCall(socketHandle, eventHandle, FrozenVram67Protocol.BuildPrime(), "PRIME_0X6E");
            sends++;
            Thread.Sleep(FrozenVram67Protocol.InterPacketDelayMs);

            SendSingleCall(socketHandle, eventHandle, FrozenVram67Protocol.BuildVoiceTip(), "VOICETIP_0XA5");
            sends++;
            Thread.Sleep(FrozenVram67Protocol.InterPacketDelayMs);

            SendSingleCall(socketHandle, eventHandle, FrozenVram67Protocol.BuildOverwrite(), "OVERWRITE_0X6C");
            sends++;
            customOverwriteSends = 1;
            ...
            return new Result(connectionsAttempted, sends, customOverwriteSends, observed.ElapsedMilliseconds, true);
        }
    ...
    private static void WaitForInitialWritable(IntPtr socketHandle, IntPtr eventHandle, Deadline deadline)
    {
        while (true)
        {
            ...
            if ((events.NetworkEvents & FdClose) != 0)
                throw new InvalidOperationException($"VRAM67_INITIAL_WRITE_WINDOW_CLOSED WSA={events.ErrorCodes[FdCloseBit]}; NO_RETRY");
            if ((events.NetworkEvents & FdWrite) == 0) continue;
            if (events.ErrorCodes[FdWriteBit] != 0)
                throw new InvalidOperationException($"VRAM67_INITIAL_WRITE_WINDOW_FAILED WSA={events.ErrorCodes[FdWriteBit]}; NO_RETRY");
            return;
        }
    }
    ...
    private static void SendSingleCall(IntPtr socketHandle, IntPtr eventHandle, byte[] bytes, string stage)
    {
        ...
        if (sent != bytes.Length)
        {
            ...
            throw new InvalidOperationException($"VRAM67_{stage}_SEND_{suffix} BYTES={sent}/{bytes.Length} WSA={error}; NO_RETRY");
        }
    }
    ...
    private static void CheckNoCloseEvent(IntPtr socketHandle, IntPtr eventHandle, string stage)
    {
        ...
        if (wait == WsaWaitFailed)
            throw new InvalidOperationException($"VRAM67_{stage}_PRE_SEND_WAIT_FAILED WSA={WSAGetLastError()}; NO_RETRY");
        if (wait != WsaWaitEvent0)
            throw new InvalidOperationException($"VRAM67_{stage}_PRE_SEND_WAIT_UNEXPECTED RESULT={wait}; NO_RETRY");
        var events = WsaNetworkEvents.Create();
        if (WSAEnumNetworkEvents(socketHandle, eventHandle, ref events) == SocketError)
            throw new InvalidOperationException($"VRAM67_{stage}_PRE_SEND_ENUM_FAILED WSA={WSAGetLastError()}; NO_RETRY");
        if ((events.NetworkEvents & FdClose) != 0)
            throw new InvalidOperationException($"VRAM67_{stage}_CLOSED WSA={events.ErrorCodes[FdCloseBit]}; NO_RETRY");
        if ((events.NetworkEvents & FdWrite) != 0 && events.ErrorCodes[FdWriteBit] != 0)
            throw new InvalidOperationException($"VRAM67_{stage}_WRITE_EVENT_FAILED WSA={events.ErrorCodes[FdWriteBit]}; NO_RETRY");
    }
