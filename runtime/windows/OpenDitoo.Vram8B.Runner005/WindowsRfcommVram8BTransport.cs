...
internal static class WindowsRfcommVram8BTransport
{
        ...
        try
        {
            ...
            for (var i = 0; i < FrozenVram8BProtocol.Fixtures.Length; i++)
            {
                ...
                switch (f.Key)
                {
                    ...
                    case "restore_set":
                    {
                        var payload = ReceiveRequiredMatching(socketHandle, eventHandle, 0xB2, null, f.Key.ToUpperInvariant());
                        if (payload.Length != 1)
                            throw new InvalidOperationException($"VRAM8B_B2_RESPONSE_LENGTH_REJECTED LENGTH={payload.Length}; NO_RETRY");
                        break;
                    }
                    case "baseline_get":
                    case "post_get":
                    case "restore_get":
                    {
                        var payload = ReceiveRequiredMatching(socketHandle, eventHandle, 0xB3, (byte)f.ExpectedB3Value!.Value, f.Key.ToUpperInvariant());
                        var value = payload[0];
                    ...
                    case "prime":
                        // Live 002 proved the exact stock 0x6e send completes but produces no wrapped
                        // response within 2000 ms in this state. Do not create a false receive gate.
                        break;
                    case "voicetip":
                        DrainOptionalValidReports(socketHandle, eventHandle, "VOICETIP_0XA5");
                        break;
                    case "overwrite":
                        DrainOptionalValidReports(socketHandle, eventHandle, "OVERWRITE_0X6C");
                        break;
                    default:
    ...
    private static byte[] ReceiveRequiredMatching(IntPtr socketHandle, IntPtr eventHandle, byte expectedCommand, byte? exactSinglePayload, string stage)
    {
        var deadline = new Deadline(FrozenVram8BProtocol.ResponseBudgetMs);
        for (var skipped = 0; skipped <= FrozenVram8BProtocol.MaxUnrelatedValidFrames; skipped++)
        {
            var response = ReceiveOneWrapped(socketHandle, eventHandle, deadline, stage + "_MATCH");
            FrozenVram8BProtocol.ValidateWrappedResponse(response, response[4], null);
            if (response[4] != expectedCommand)
            {
                Console.WriteLine($"VRAM8B_VALID_UNRELATED_SKIPPED stage={stage} inner=0x{response[4]:X2} count={skipped + 1}");
                if (skipped == FrozenVram8BProtocol.MaxUnrelatedValidFrames)
                    throw new InvalidOperationException($"VRAM8B_{stage}_UNRELATED_FRAME_BUDGET_EXHAUSTED; NO_RETRY");
                continue;
            }
            return FrozenVram8BProtocol.ValidateWrappedResponse(response, expectedCommand, exactSinglePayload);
        }
        throw new InvalidOperationException($"VRAM8B_{stage}_MATCH_UNREACHABLE; NO_RETRY");
    }

    private static int DrainOptionalValidReports(IntPtr socketHandle, IntPtr eventHandle, string stage)
    {
        var deadline = new Deadline(FrozenVram8BProtocol.OptionalDrainQuietMs);
        var drained = 0;
        while (drained < FrozenVram8BProtocol.OptionalDrainMaxFrames)
        {
            try
            {
                var response = ReceiveOneWrapped(socketHandle, eventHandle, deadline, stage + "_DRAIN");
                FrozenVram8BProtocol.ValidateWrappedResponse(response, response[4], null);
                drained++;
                Console.WriteLine($"VRAM8B_VALID_REPORT_DRAINED stage={stage} inner=0x{response[4]:X2} count={drained}");
            }
