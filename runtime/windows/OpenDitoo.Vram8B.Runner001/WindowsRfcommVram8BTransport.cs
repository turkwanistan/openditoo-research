...
internal static class WindowsRfcommVram8BTransport
{
    ...
    internal static Result RunExactlyOnce(byte[][] fixtures)
    {
        ...
        try
        {
            ...
            for (var i = 0; i < FrozenVram8BProtocol.Fixtures.Length; i++)
            {
                ...
                if (f.IsB3Query)
                {
                    var response = ReceiveOneB3(socketHandle, eventHandle, new Deadline(FrozenVram8BProtocol.ResponseBudgetMs), f.Key.ToUpperInvariant());
                    var value = FrozenVram8BProtocol.ValidateB3Response(response, f.ExpectedB3Value!.Value);
                    if (f.Key == "baseline_get") baseline = value;
                    else if (f.Key == "post_get") post = value;
                    else if (f.Key == "restore_get") restored = value;
                }
    ...
    private static byte[] ReceiveOneB3(IntPtr socketHandle, IntPtr eventHandle, Deadline deadline, string stage)
    {
        SelectEvents(socketHandle, eventHandle, FdRead | FdClose, stage + "_RECV");
        var received = new List<byte>(FrozenVram8BProtocol.ExpectedB3WireBytes);
        var expectedTotal = -1;
        var chunk = new byte[FrozenVram8BProtocol.ExpectedB3WireBytes];
        while (true)
        {
            ...
            var remaining = FrozenVram8BProtocol.ExpectedB3WireBytes - received.Count;
            ...
            if (received.Count >= 3 && expectedTotal < 0)
            {
                ...
                if (expectedTotal != FrozenVram8BProtocol.ExpectedB3WireBytes)
