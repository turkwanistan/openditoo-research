...
- **Pacing (superseded by R1-R5 and the later product integration):** future activity-session source uses a 150 ms frame-start floor (6.67 fps) and the physically accepted 10 ms intra-frame spacing. R4 sustained 7.63 fps full-colour and R5 established an 18.46 fps one-ACK-per-frame ceiling; 150 ms is intentionally slower for jitter headroom. Unchanged scenes still send nothing. Pipelining remains out of scope.
...
| Faster pacing / animation / video | **Rate question closed for this app** | R1-R5 measured 18.46 fps ceiling; product source is wired at 150 ms / 6.67 fps with the four-stage pulse. Do not pursue pipelining or delta/partial frames absent a new concrete requirement. |
