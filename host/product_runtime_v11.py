"""Runtime 017 successor: Runtime 017 pixels/runtime with media-resilient raw AVRCP input.

Runtime 017 remains the rollback baseline. Runtime 017 changes only the receive-side Windows input seam:
a forced-Playing SMTC sink holds media-session ownership while a BTVS/tshark sidecar attributes raw AVRCP
PASS THROUGH presses to the exact Ditoo and emits normalized NDJSON. 0x44 Play and 0x46 Pause are both
one short-lever action; 0x4C/0x4B remain Left/Right. Display transport and page/game semantics are unchanged.

Runtime 015 remains the live rollback baseline. Runtime 017 preserves all Runtime 015 behavior and uses
text-authored Pixel Animation Lab assets and page-local selector navigation. It changes no Host,
Bluetooth target, pacing profile, reconnect/reclaim behavior, ButtonProbe contract or webcam policy.
...
HASHED_MODULES = {
    "product_runtime_v11_sha256": ROOT / "host/product_runtime_v11.py",
...
def code_hashes() -> dict[str, str]:
    ...
    # then pin every module and authored asset that can change Runtime 017 pixels, input or pacing.
...
def load_policy(path: Path, *, require_authority: bool = True, verify_hashes: bool = True,
    ...
    try:
        ...
        r016 = json.loads(R016_TEMPLATE.read_text(encoding="utf-8"))
    ...
    _require(raw.get("pages") == PAGES, "PRODUCT_017_PAGES_MISMATCH")
    _require(raw.get("session") == SESSION, "PRODUCT_017_ENVELOPE_MISMATCH")
    for key in ("target", "behavior", "product_id", "install"):
        _require(raw.get(key) == r016[key], "PRODUCT_017_WIDENS_016", key)
...
def session_manifest(policy: StandingPolicy, experiment_id: str) -> activity_session.SessionManifest:
    return activity_session.SessionManifest(
        ...
        activation_source="persistent Runtime 017 media-resilient raw-AVRCP interactive product",
