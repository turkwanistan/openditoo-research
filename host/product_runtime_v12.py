"""Runtime 018 successor: Runtime 017 with a hidden capture window and self-healing input binding.

Runtime 017 is the rollback baseline. Runtime 018 keeps its pixels, pages, Host, session envelope, pacing,
reconnect/reclaim, ButtonProbe ownership sink and raw-AVRCP input contract, and changes only two things:
- the elevated sidecar starts BTVS with its window hidden (and re-hides it), because closing that window killed
  the capture and restarted the sidecar (owner, 2026-09-12);
- a (re)started sidecar learns the Ditoo ACL handle only from a session's first frame, and an idle dashboard is
  change-only, so when the broker is capture-ready but unbound for REBIND_UNBOUND_SECONDS during a connected
  session, the runtime performs ONE clean session renewal per broker epoch (the proven lifetime-rollover path);
  the new session's first frame rebinds input.
The sidecar runs from its own admin-only install root and task so Runtime 017's installed bytes stay intact as
the exact rollback.

Supervisor/session semantics otherwise remain Runtime 017's (see ``product_runtime_v11``).
...
from host import activity_session, frame_stream, interactive_stream, mcp_activity, pagination, product_runtime_v2, raw_avrcp_input
...
from host.moss_page import MossPage

ROOT = Path(__file__).resolve().parents[1]
RUNTIME_REVISION = 13
EXACT_UNIT_ID = activity_session.EXACT_UNIT_ID
TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-018.json"
R017_TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-017.json"
GRANT_TEXT = "Grant OPENDITOO-PRODUCT-RUNTIME-018"
RAW_AVRCP_TASK = "OpenDitoo Raw AVRCP Broker 018"
RAW_AVRCP_INSTALL_ROOT = r"C:\Program Files\OpenDitoo\RawAvrcpBroker018"  # inherits admin-only ACL
REBIND_UNBOUND_SECONDS = 3.0  # capture-ready but no Ditoo handle this long in a connected session -> renew once
...
PAGES = [
    ...
    {"name": "moss", "kind": "pocket_moss_v1", "rate_mode": frame_stream.SESSION_PROFILE_STREAMING,
     "lever": "normal: enter selector; selector: execute Pet/Dance/Kisses/Back"},
]
...
HASHED_MODULES = {
    "product_runtime_v12_sha256": ROOT / "host/product_runtime_v12.py",
    "raw_avrcp_input_sha256": ROOT / "host/raw_avrcp_input.py",
    ...
    "runtime_018_cutover_sha256": ROOT / "scripts/cutover_runtime_018.sh",
    ...
    "moss_page_sha256": ROOT / "host/moss_page.py",
    "pixel_art_sha256": ROOT / "host/pixel_art.py",
    "pixel_animation_sha256": ROOT / "host/pixel_animation.py",
    "moss_v4_asset_animations_dance_sha256": ROOT / "assets/pocket_moss/v4/animations/dance.json",
    "moss_v4_asset_animations_idle_look_sha256": ROOT / "assets/pocket_moss/v4/animations/idle-look.json",
    "moss_v4_asset_animations_kisses_sha256": ROOT / "assets/pocket_moss/v4/animations/kisses.json",
    "moss_v4_asset_animations_loaf_sha256": ROOT / "assets/pocket_moss/v4/animations/loaf.json",
    "moss_v4_asset_animations_look_sha256": ROOT / "assets/pocket_moss/v4/animations/look.json",
    "moss_v4_asset_animations_pet_sha256": ROOT / "assets/pocket_moss/v4/animations/pet.json",
    "moss_v4_asset_animations_sleep_wake_sha256": ROOT / "assets/pocket_moss/v4/animations/sleep-wake.json",
    "moss_v4_asset_sprites_blink_sha256": ROOT / "assets/pocket_moss/v4/sprites/blink.json",
    "moss_v4_asset_sprites_dance_crouch_sha256": ROOT / "assets/pocket_moss/v4/sprites/dance_crouch.json",
    "moss_v4_asset_sprites_dance_hop_sha256": ROOT / "assets/pocket_moss/v4/sprites/dance_hop.json",
    "moss_v4_asset_sprites_dance_land_sha256": ROOT / "assets/pocket_moss/v4/sprites/dance_land.json",
    "moss_v4_asset_sprites_dance_left_sha256": ROOT / "assets/pocket_moss/v4/sprites/dance_left.json",
    "moss_v4_asset_sprites_dance_right_sha256": ROOT / "assets/pocket_moss/v4/sprites/dance_right.json",
    "moss_v4_asset_sprites_idle_sha256": ROOT / "assets/pocket_moss/v4/sprites/idle.json",
    "moss_v4_asset_sprites_kiss_big_sha256": ROOT / "assets/pocket_moss/v4/sprites/kiss_big.json",
    "moss_v4_asset_sprites_kiss_lean_sha256": ROOT / "assets/pocket_moss/v4/sprites/kiss_lean.json",
    "moss_v4_asset_sprites_kiss_pop_sha256": ROOT / "assets/pocket_moss/v4/sprites/kiss_pop.json",
    "moss_v4_asset_sprites_loaf_sha256": ROOT / "assets/pocket_moss/v4/sprites/loaf.json",
    "moss_v4_asset_sprites_loaf_blink_sha256": ROOT / "assets/pocket_moss/v4/sprites/loaf_blink.json",
    "moss_v4_asset_sprites_look_left_sha256": ROOT / "assets/pocket_moss/v4/sprites/look_left.json",
    "moss_v4_asset_sprites_look_right_sha256": ROOT / "assets/pocket_moss/v4/sprites/look_right.json",
    "moss_v4_asset_sprites_perk_sha256": ROOT / "assets/pocket_moss/v4/sprites/perk.json",
    "moss_v4_asset_sprites_pet_lean_sha256": ROOT / "assets/pocket_moss/v4/sprites/pet_lean.json",
    "moss_v4_asset_sprites_pet_squish_sha256": ROOT / "assets/pocket_moss/v4/sprites/pet_squish.json",
    "moss_v4_asset_sprites_sleep_a_sha256": ROOT / "assets/pocket_moss/v4/sprites/sleep_a.json",
    "moss_v4_asset_sprites_sleep_b_sha256": ROOT / "assets/pocket_moss/v4/sprites/sleep_b.json",
    "moss_v4_asset_sprites_wag_high_sha256": ROOT / "assets/pocket_moss/v4/sprites/wag_high.json",
    "moss_v4_asset_sprites_wag_low_sha256": ROOT / "assets/pocket_moss/v4/sprites/wag_low.json",
    "moss_v4_asset_sprites_yawn_sha256": ROOT / "assets/pocket_moss/v4/sprites/yawn.json",
}
...
def code_hashes() -> dict[str, str]:
    # Reuse Runtime 009's unchanged supervisor/renderer/collector stack via product_runtime_v2,
    # then pin every module and authored asset that can change Runtime 018 pixels, input or pacing.
...
def load_policy(path: Path, *, require_authority: bool = True, verify_hashes: bool = True,
    ...
    try:
        ...
        r017 = json.loads(R017_TEMPLATE.read_text(encoding="utf-8"))
    ...
    _require(raw.get("pages") == PAGES, "PRODUCT_018_PAGES_MISMATCH")
    _require(raw.get("session") == SESSION, "PRODUCT_018_ENVELOPE_MISMATCH")
    for key in ("target", "behavior", "product_id", "install"):
        _require(raw.get(key) == r017[key], "PRODUCT_018_WIDENS_017", key)
    broker_spec = (raw.get("pagination") or {}).get("broker") or {}
    _require(broker_spec.get("kind") == "raw_avrcp_v1"
             and broker_spec.get("sink_status") == "playing"
             and broker_spec.get("port") == 24353
             and broker_spec.get("operations") == {
                 "0x44": "lever_candidate", "0x46": "lever_candidate",
                 "0x4B": "nav_right", "0x4C": "nav_left"},
             "PRODUCT_018_INPUT_CONTRACT_MISMATCH")
    ...
    _require(launch.get("kind") == "scheduled_task_highest_on_demand" and launch.get("task_name") == RAW_AVRCP_TASK
             ...
             "PRODUCT_018_ELEVATED_LAUNCH_MISMATCH")
    _require((raw.get("build") or {}).get("button_probe_sha256") == r017["build"]["button_probe_sha256"]
             and (raw.get("build") or {}).get("host_dll_sha256") == r017["build"]["host_dll_sha256"],
             "PRODUCT_018_CHANGES_HOST_OR_SINK")
    ...
    if verify_hashes:
        ...
        for group, code in (("raw_avrcp_broker_sha256", "PRODUCT_RAW_AVRCP_BROKER_HASH_MISMATCH"),
                            ("raw_avrcp_dependencies_sha256", "PRODUCT_RAW_AVRCP_DEPENDENCY_HASH_MISMATCH")):
            for name, digest in build[group].items():
                _require(_sha(pagination.windows_path(name)) == digest, code, name)
...
def session_manifest(policy: StandingPolicy, experiment_id: str) -> activity_session.SessionManifest:
    return activity_session.SessionManifest(
        ...
        activation_source="persistent Runtime 018 media-resilient raw-AVRCP interactive product",
...
class _RebindGuard:
    """Ask for ONE clean session renewal per broker epoch when capture is up but the Ditoo handle is unknown.

    A (re)started sidecar learns the handle only from a session's first frame and an idle dashboard is
    change-only, so without this a sidecar restart left input dead until the 600 s rollover (Runtime 017 live,
    2026-09-12). Once per epoch: if a renewal does not bind, the next genuine session boundary still will.
    """

    def __init__(self, events, monotonic: Callable[[], float]) -> None:
        self.events, self.monotonic = events, monotonic
    ...
    def due(self) -> bool:
        if self.fired:
            return True  # stays requested until the supervisor consumes it after the session closes
        e = self.events
        if (not isinstance(e, raw_avrcp_input.RawAvrcpEvents) or e.bound or not e.capture_ready
                or e.epoch == self.done_epoch):
            self.since = None
            return False
        now = self.monotonic()
        if self.since is None:
            self.since = now
            return False
        if now - self.since < REBIND_UNBOUND_SECONDS:
            return False
        self.done_epoch, self.since, self.fired, self.count = e.epoch, None, True, self.count + 1
        return True
...
def run_product(policy: StandingPolicy, transport_factory, *, stop_requested: Callable[[], bool] = lambda: False,
    ...
    rebind = _RebindGuard(events, monotonic)
    ...
    def publish() -> None:
        ...
        moss = next((p.telemetry() for p in carousel.pages if p.name == "moss"), None)
        state = {"updated_at": _utc(), "runtime_revision": RUNTIME_REVISION,
                 ...
                 "input_rebind_renewals": rebind.count,
                 ...
                 "moss": None if moss is None else {k: moss[k] for k in ("mode", "behavior_state", "active_animation",
                                                                         "selected_action", "interactions", "completed_actions")}}
    ...
    def frame_acked(_ack: dict, count: int) -> None:
        ...
        if fresh:
            ...
            publish()
        # count == 1: a session's first ACK often lands < 1 s after the open's persist (Runtime 009
    ...
    try:
        while not stop_requested():
            ...
            result = interactive_stream.run_interactive_stream(
                ...
                transport, clock, sleep_ms, driver, stop_requested=lambda: stop_requested() or rebind.due(),
            ...
            if rebind.fired and not stop_requested():
                # Our own clean close to rebind input: renew like a lifetime rollover; the page resumes untouched.
                rebind.fired = False
