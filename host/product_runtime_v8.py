"""Runtime 014 successor: Runtime 013 with the Slots v2 polish (snap-to-symbol stops, win rules/animation, shaded art).

Runtime 013 remains the live rollback baseline. Runtime 014 preserves all Runtime 013 behavior and uses
...
TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-014.json"
R013_TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-013.json"
GRANT_TEXT = "Grant OPENDITOO-PRODUCT-RUNTIME-014"
...
HASHED_MODULES = {
    "product_runtime_v8_sha256": ROOT / "host/product_runtime_v8.py",
...
def code_hashes() -> dict[str, str]:
    ...
    # then pin every module and authored asset that can change Runtime 014 pixels, input or pacing.
...
def load_policy(path: Path, *, require_authority: bool = True, verify_hashes: bool = True,
    ...
    try:
        ...
        r013 = json.loads(R013_TEMPLATE.read_text(encoding="utf-8"))
    ...
    _require(raw.get("pages") == PAGES, "PRODUCT_014_PAGES_MISMATCH")
    _require(raw.get("session") == SESSION, "PRODUCT_014_ENVELOPE_MISMATCH")
    for key in ("target", "behavior", "product_id", "install"):
        _require(raw.get(key) == r013[key], "PRODUCT_014_WIDENS_013", key)
    _require((raw.get("pagination") or {}).get("broker") == r013["pagination"]["broker"]
             and (raw.get("build") or {}).get("button_probe_sha256") == r013["build"]["button_probe_sha256"]
             and (raw.get("build") or {}).get("host_dll_sha256") == r013["build"]["host_dll_sha256"],
             "PRODUCT_014_CHANGES_INPUT_OR_HOST")
...
def session_manifest(policy: StandingPolicy, experiment_id: str) -> activity_session.SessionManifest:
    return activity_session.SessionManifest(
        ...
        activation_source="persistent Runtime 014 Slots v2 interactive product",
