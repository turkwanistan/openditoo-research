"""Runtime 015 successor: Runtime 014 with Slots v2 aim tuning (nearest-symbol snap, slower middle/right reels).

Runtime 014 remains the live rollback baseline. Runtime 015 preserves all Runtime 014 behavior and uses
...
TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-015.json"
R014_TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-014.json"
GRANT_TEXT = "Grant OPENDITOO-PRODUCT-RUNTIME-015"
...
HASHED_MODULES = {
    "product_runtime_v9_sha256": ROOT / "host/product_runtime_v9.py",
...
def code_hashes() -> dict[str, str]:
    ...
    # then pin every module and authored asset that can change Runtime 015 pixels, input or pacing.
...
def load_policy(path: Path, *, require_authority: bool = True, verify_hashes: bool = True,
    ...
    try:
        ...
        r014 = json.loads(R014_TEMPLATE.read_text(encoding="utf-8"))
    ...
    _require(raw.get("pages") == PAGES, "PRODUCT_015_PAGES_MISMATCH")
    _require(raw.get("session") == SESSION, "PRODUCT_015_ENVELOPE_MISMATCH")
    for key in ("target", "behavior", "product_id", "install"):
        _require(raw.get(key) == r014[key], "PRODUCT_015_WIDENS_014", key)
    _require((raw.get("pagination") or {}).get("broker") == r014["pagination"]["broker"]
             and (raw.get("build") or {}).get("button_probe_sha256") == r014["build"]["button_probe_sha256"]
             and (raw.get("build") or {}).get("host_dll_sha256") == r014["build"]["host_dll_sha256"],
             "PRODUCT_015_CHANGES_INPUT_OR_HOST")
...
def session_manifest(policy: StandingPolicy, experiment_id: str) -> activity_session.SessionManifest:
    return activity_session.SessionManifest(
        ...
        activation_source="persistent Runtime 015 Slots v2 interactive product",
