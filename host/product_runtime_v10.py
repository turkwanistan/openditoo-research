"""Runtime 016 successor: Runtime 015 with smooth Slots reels (steady 1 px/frame, per-reel slip-back).

Runtime 015 remains the live rollback baseline. Runtime 016 preserves all Runtime 015 behavior and uses
...
TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-016.json"
R015_TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-015.json"
GRANT_TEXT = "Grant OPENDITOO-PRODUCT-RUNTIME-016"
...
HASHED_MODULES = {
    "product_runtime_v10_sha256": ROOT / "host/product_runtime_v10.py",
...
def code_hashes() -> dict[str, str]:
    ...
    # then pin every module and authored asset that can change Runtime 016 pixels, input or pacing.
...
def load_policy(path: Path, *, require_authority: bool = True, verify_hashes: bool = True,
    ...
    try:
        ...
        r015 = json.loads(R015_TEMPLATE.read_text(encoding="utf-8"))
    ...
    _require(raw.get("pages") == PAGES, "PRODUCT_016_PAGES_MISMATCH")
    _require(raw.get("session") == SESSION, "PRODUCT_016_ENVELOPE_MISMATCH")
    for key in ("target", "behavior", "product_id", "install"):
        _require(raw.get(key) == r015[key], "PRODUCT_016_WIDENS_015", key)
    _require((raw.get("pagination") or {}).get("broker") == r015["pagination"]["broker"]
             and (raw.get("build") or {}).get("button_probe_sha256") == r015["build"]["button_probe_sha256"]
             and (raw.get("build") or {}).get("host_dll_sha256") == r015["build"]["host_dll_sha256"],
             "PRODUCT_016_CHANGES_INPUT_OR_HOST")
...
def session_manifest(policy: StandingPolicy, experiment_id: str) -> activity_session.SessionManifest:
    return activity_session.SessionManifest(
        ...
        activation_source="persistent Runtime 016 Slots v2 interactive product",
