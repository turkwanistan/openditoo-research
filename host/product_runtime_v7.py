"""Runtime 013 successor: Runtime 012 with the Pocket Moss v4 art/animation pass.

Runtime 012 remains the live rollback baseline. Runtime 013 preserves all Runtime 012 behavior and uses
...
TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-013.json"
R012_TEMPLATE = ROOT / "product" / "OPENDITOO-PRODUCT-RUNTIME-012.json"
GRANT_TEXT = "Grant OPENDITOO-PRODUCT-RUNTIME-013"
...
HASHED_MODULES = {
    "product_runtime_v7_sha256": ROOT / "host/product_runtime_v7.py",
...
def code_hashes() -> dict[str, str]:
    ...
    # then pin every module and authored asset that can change Runtime 013 pixels, input or pacing.
...
def load_policy(path: Path, *, require_authority: bool = True, verify_hashes: bool = True,
    ...
    try:
        ...
        r012 = json.loads(R012_TEMPLATE.read_text(encoding="utf-8"))
    ...
    _require(raw.get("pages") == PAGES, "PRODUCT_013_PAGES_MISMATCH")
    _require(raw.get("session") == SESSION, "PRODUCT_013_ENVELOPE_MISMATCH")
    for key in ("target", "behavior", "product_id", "install"):
        _require(raw.get(key) == r012[key], "PRODUCT_013_WIDENS_012", key)
    _require((raw.get("pagination") or {}).get("broker") == r012["pagination"]["broker"]
             and (raw.get("build") or {}).get("button_probe_sha256") == r012["build"]["button_probe_sha256"]
             and (raw.get("build") or {}).get("host_dll_sha256") == r012["build"]["host_dll_sha256"],
             "PRODUCT_013_CHANGES_INPUT_OR_HOST")
...
def session_manifest(policy: StandingPolicy, experiment_id: str) -> activity_session.SessionManifest:
    return activity_session.SessionManifest(
        ...
        activation_source="persistent Runtime 013 Pocket Moss interactive product",
