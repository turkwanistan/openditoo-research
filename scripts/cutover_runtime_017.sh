#!/usr/bin/env bash
# Runtime 017 cutover: Runtime 016 pixels/Host/session unchanged; input only -> exact-target raw AVRCP.
#   bash scripts/cutover_runtime_017.sh "Grant OPENDITOO-PRODUCT-RUNTIME-017"
#   bash scripts/cutover_runtime_017.sh --rollback
...
cd "$MAIN"
LOCAL=.openditoo-local; POLICY=$LOCAL/product-runtime-policy.json; RB=$LOCAL/rollback-runtime-016
TEMPLATE="$HERE/product/OPENDITOO-PRODUCT-RUNTIME-017.json"
...
raw=json.loads(v11.TEMPLATE.read_text(encoding='utf-8'))
assert '__PENDING_' not in json.dumps(raw), 'Runtime 017 binary/dependency identities are not frozen'
v11.load_policy(v11.TEMPLATE, require_authority=False, verify_hashes=False)
assert raw['runtime_revision'] == 12
assert raw['build']['code_sha256'] == v11.code_hashes()
assert v11.authority_blockers(v11.TEMPLATE) == [
  'PRODUCT_AUTHORITY_MISSING','PRODUCT_AUTHORITY_UNATTRIBUTED','PRODUCT_AUTHORITY_SCOPE_MISMATCH']
print('R017_TEMPLATE_READY')
PY
}
...
start_and_confirm(){
  ...
  echo "R017_DASHBOARD_NOT_CONFIRMED $r" >&2; return 1
}
product_check(){
...
if [[ "${1:-}" == "--rollback" ]]; then
  ...
  if [[ "$(git rev-parse 'HEAD^{tree}')" != "$(git rev-parse "$pre^{tree}")" ]]; then
    ...
    git commit -q -m "Rollback: restore the Runtime 016 source tree ($(git rev-parse --short "$pre"))"
  fi
...
[[ "${1:-}" == "Grant OPENDITOO-PRODUCT-RUNTIME-017" ]] || { echo R017_EXACT_GRANT_REQUIRED >&2; exit 2; }
[[ -z "$(git status --porcelain --untracked-files=no)" ]] || { echo R017_MAIN_HAS_TRACKED_CHANGES >&2; exit 2; }
git merge-base --is-ancestor HEAD "$BRANCH" || { echo R017_BRANCH_NOT_FAST_FORWARD >&2; exit 2; }
[[ "$(policy_id "$POLICY")" == OPENDITOO-PRODUCT-RUNTIME-016 ]] || { echo R017_CURRENT_POLICY_IS_NOT_016 >&2; exit 2; }
template_ready || { echo R017_TEMPLATE_NOT_READY >&2; exit 2; }
...
python3 -c 'from host import webcam_trial; webcam_trial._host_preclaim_status()' \
  || { echo R017_HOST_NOT_IDLE >&2; systemctl --user start "$SERVICE"; exit 2; }
echo R017_HOST_IDLE
rollback_now(){ echo "R017_FAILED: $1 -- rolling back" >&2; bash "$HERE/scripts/cutover_runtime_017.sh" --rollback; exit 2; }
...
echo R017_MAIN_OFFLINE_PASS

python3 - "$1" <<'PY' || rollback_now LOCAL_POLICY
import json,os,sys
from pathlib import Path
doc=json.loads(Path('product/OPENDITOO-PRODUCT-RUNTIME-017.json').read_text(encoding='utf-8'))
a=doc['authority']
...
os.replace(tmp,path)
print('R017_LOCAL_POLICY_WRITTEN')
PY
product_check 12 && echo R017_PRODUCT_CHECK_PASS || rollback_now PRODUCT_CHECK
start_and_confirm || rollback_now DASHBOARD_NOT_CONNECTED
python3 host/webcam_studio.py policy-check >/dev/null && echo R017_WEBCAM_POLICY_CHECK_PASS || rollback_now WEBCAM_POLICY_CHECK
echo "R017_CUTOVER_PASS main=$(git rev-parse --short HEAD) rollback=$RB"
