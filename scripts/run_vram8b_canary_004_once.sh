...
cd "$ROOT"
EXPERIMENT_ID=OPENDITOO-VRAM8B-CANARY-004
GRANT_TEXT='Grant OPENDITOO-VRAM8B-CANARY-004 -- stock btplayer selected'
MANIFEST="experiments/${EXPERIMENT_ID}.json"
FIXTURE_REPORT=artifacts/analysis/volatile_ram_api_vram8b_fixture_004.json
EXPECTED_MANIFEST_SHA=0906626243dd1859ce52cef62261ddee0bd14df72043c966d3a06b8665a26c24
EXPECTED_FIXTURE_SHA=b85b0f104d6ed8b1d2781080bb8fa3acbe1a7c89c06d90a1bfd842e1564b9e2e
...
LOCAL_DIR=.openditoo-local/vram8b-canary-004
...
RUNNER_PROJECT=runtime/windows/OpenDitoo.Vram8B.Runner004/OpenDitoo.Vram8B.Runner004.csproj
RUNNER_DLL=runtime/windows/OpenDitoo.Vram8B.Runner004/bin/Release/net8.0/OpenDitoo.Vram8B.Runner004.dll
PS1=runtime/windows/run_openditoo_vram8b_canary_004_once.ps1
...
python3 tools/ditoo_vram8b_live_gate_004.py --selfcheck >/dev/null
...
files=['runtime/windows/OpenDitoo.Vram8B.Runner004/OpenDitoo.Vram8B.Runner004.csproj','runtime/windows/OpenDitoo.Vram8B.Runner004/FrozenVram8BProtocol.cs','runtime/windows/OpenDitoo.Vram8B.Runner004/WindowsRfcommVram8BTransport.cs','runtime/windows/OpenDitoo.Vram8B.Runner004/Program.cs','runtime/windows/run_openditoo_vram8b_canary_004_once.ps1','scripts/run_vram8b_canary_004_once.sh']
sha=lambda x: hashlib.sha256(open(x,'rb').read()).hexdigest()
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-004','manifest_sha256':'0906626243dd1859ce52cef62261ddee0bd14df72043c966d3a06b8665a26c24','fixture_report_sha256':'b85b0f104d6ed8b1d2781080bb8fa3acbe1a7c89c06d90a1bfd842e1564b9e2e','one_use_nonce':nonce,'assembly_sha256':sha(dll),'source_files':[{'path':x,'sha256':sha(x)} for x in files],'sealed_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
...
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-004','manifest_sha256':mh,'one_use_nonce':nonce,'runtime018_service_inactive':True,'created_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
tmp=p+'.tmp'; open(tmp,'w',encoding='utf-8').write(json.dumps(d,indent=2,sort_keys=True)+'\n'); os.replace(tmp,p)
PY
