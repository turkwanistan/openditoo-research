...
cd "$ROOT"
EXPERIMENT_ID=OPENDITOO-VRAM8B-CANARY-005
GRANT_TEXT='Grant OPENDITOO-VRAM8B-CANARY-005 -- stock btplayer selected'
MANIFEST="experiments/${EXPERIMENT_ID}.json"
FIXTURE_REPORT=artifacts/analysis/volatile_ram_api_vram8b_fixture_005.json
EXPECTED_MANIFEST_SHA=0248438e5b448858a31ed64d266a84314291332022f149e9bd25a4c99e37cc57
EXPECTED_FIXTURE_SHA=43b55d41162dc12c4f98f8923fa5269affca28d3cdb1e3d96d0015630159e395
...
LOCAL_DIR=.openditoo-local/vram8b-canary-005
...
RUNNER_PROJECT=runtime/windows/OpenDitoo.Vram8B.Runner005/OpenDitoo.Vram8B.Runner005.csproj
RUNNER_DLL=runtime/windows/OpenDitoo.Vram8B.Runner005/bin/Release/net8.0/OpenDitoo.Vram8B.Runner005.dll
PS1=runtime/windows/run_openditoo_vram8b_canary_005_once.ps1
...
python3 tools/ditoo_vram8b_live_gate_005.py --selfcheck >/dev/null
...
files=['runtime/windows/OpenDitoo.Vram8B.Runner005/OpenDitoo.Vram8B.Runner005.csproj','runtime/windows/OpenDitoo.Vram8B.Runner005/FrozenVram8BProtocol.cs','runtime/windows/OpenDitoo.Vram8B.Runner005/WindowsRfcommVram8BTransport.cs','runtime/windows/OpenDitoo.Vram8B.Runner005/Program.cs','runtime/windows/run_openditoo_vram8b_canary_005_once.ps1','scripts/run_vram8b_canary_005_once.sh']
sha=lambda x: hashlib.sha256(open(x,'rb').read()).hexdigest()
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-005','manifest_sha256':'0248438e5b448858a31ed64d266a84314291332022f149e9bd25a4c99e37cc57','fixture_report_sha256':'43b55d41162dc12c4f98f8923fa5269affca28d3cdb1e3d96d0015630159e395','one_use_nonce':nonce,'assembly_sha256':sha(dll),'source_files':[{'path':x,'sha256':sha(x)} for x in files],'sealed_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
...
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-005','manifest_sha256':mh,'one_use_nonce':nonce,'runtime018_service_inactive':True,'created_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
tmp=p+'.tmp'; open(tmp,'w',encoding='utf-8').write(json.dumps(d,indent=2,sort_keys=True)+'\n'); os.replace(tmp,p)
PY
