#!/usr/bin/env bash
# Runtime 013 cutover: Runtime 012 with the Pocket Moss v4 art/animation pass only.
# Same exact Host, target, webcam policy, session envelope, pacing, reconnect/reclaim and ButtonProbe.
#   bash scripts/cutover_runtime_013.sh "Grant OPENDITOO-PRODUCT-RUNTIME-013"
#   bash scripts/cutover_runtime_013.sh --rollback     # exact source tree + local policy back: Runtime 012
# Run it from the feat/pocket-moss-v4-polish worktree. It acts on the MAIN checkout (where the
...
set -euo pipefail
BRANCH=feat/pocket-moss-v4-polish
...
[[ -n "$MAIN" && -d "$MAIN" ]] || { echo R013_MAIN_NOT_FOUND >&2; exit 2; }
cd "$MAIN"
LOCAL=.openditoo-local; POLICY=$LOCAL/product-runtime-policy.json; RB=$LOCAL/rollback-runtime-012
...
probe_ok(){
  python3 - "$HERE/product/OPENDITOO-PRODUCT-RUNTIME-013.json" <<'PY'
...
template_ok(){
  ...
  # Validate the exact Runtime 013 source/assets and policy structure here; installed Host/Probe
...
raw = json.loads(v7.TEMPLATE.read_text(encoding="utf-8"))
v7.load_policy(v7.TEMPLATE, require_authority=False, verify_hashes=False)
assert raw["runtime_revision"] == 8
assert raw["build"]["code_sha256"] == v7.code_hashes()
assert v7.authority_blockers(v7.TEMPLATE) == [
...
stop_service(){
  ...
  echo R013_SERVICE_STOP_FAILED >&2; return 1
}
start_and_confirm(){
  ...
  echo "R013_DASHBOARD_NOT_CONFIRMED $r" >&2; return 1
}
...
if [[ "${1:-}" == "--rollback" ]]; then
  [[ -f "$RB/pre_commit" && -f "$RB/product-runtime-policy.json" ]] || { echo R013_NO_ROLLBACK_SAVED >&2; exit 2; }
  stop_service
  pre="$(cat "$RB/pre_commit")"
  # The merged range may contain merge commits, which `git revert A..B` cannot revert. Restore the
  # exact Runtime 012 tree as one forward commit instead (no history rewrite), then prove it.
  if [[ "$(git rev-parse 'HEAD^{tree}')" != "$(git rev-parse "$pre^{tree}")" ]]; then
    git read-tree -u --reset "$pre"
    git commit -q -m "Rollback: restore the Runtime 012 source tree ($(git rev-parse --short "$pre"))"
  fi
  [[ "$(git rev-parse 'HEAD^{tree}')" == "$(git rev-parse "$pre^{tree}")" ]] || { echo R013_ROLLBACK_TREE_MISMATCH >&2; exit 2; }
  install -m 600 "$RB/product-runtime-policy.json" "$POLICY"
  host_ok || { echo R013_ROLLBACK_HOST_HASH >&2; exit 2; }
  product_check 7 && echo R013_ROLLBACK_PRODUCT_CHECK_PASS
  start_and_confirm && echo R013_ROLLED_BACK_TO_RUNTIME_012
  exit 0
fi

[[ "${1:-}" == "Grant OPENDITOO-PRODUCT-RUNTIME-013" ]] || { echo R013_EXACT_GRANT_REQUIRED >&2; exit 2; }
[[ -z "$(git status --porcelain --untracked-files=no)" ]] || { echo R013_MAIN_HAS_TRACKED_CHANGES >&2; exit 2; }
git merge-base --is-ancestor HEAD "$BRANCH" || { echo R013_BRANCH_NOT_FAST_FORWARD >&2; exit 2; }
[[ "$(policy_id "$POLICY")" == OPENDITOO-PRODUCT-RUNTIME-012 ]] || { echo R013_CURRENT_POLICY_IS_NOT_012 >&2; exit 2; }
host_ok || { echo R013_INSTALLED_HOST_NOT_RUNTIME_012 >&2; exit 2; }
probe_ok || { echo R013_INSTALLED_BUTTON_PROBE_NOT_RUNTIME_012 >&2; exit 2; }
...
template_ok || { echo R013_TEMPLATE_NOT_HASH_VALID >&2; exit 2; }
echo R013_TEMPLATE_AND_INSTALLED_BINARIES_PASS
...
cp -p "$POLICY" "$RB/"
echo "R013_ROLLBACK_SAVED pre=$(cut -c1-12 "$RB/pre_commit")"

stop_service
python3 -c 'from host import webcam_trial; webcam_trial._host_preclaim_status()' \
  || { echo R013_HOST_NOT_IDLE >&2; systemctl --user start "$SERVICE"; exit 2; }
echo R013_HOST_IDLE
rollback_now(){ echo "R013_FAILED: $1 -- rolling back" >&2; bash "$HERE/scripts/cutover_runtime_013.sh" --rollback; exit 2; }

git merge --ff-only "$BRANCH" >/dev/null && echo "R013_MAIN_FAST_FORWARDED $(git rev-parse --short HEAD)"
...
echo R013_MAIN_OFFLINE_PASS

python3 - "$1" <<'PY' || rollback_now LOCAL_POLICY
import json, os, sys
from pathlib import Path
doc = json.loads(Path("product/OPENDITOO-PRODUCT-RUNTIME-013.json").read_text(encoding="utf-8"))
a = doc["authority"]
...
os.replace(tmp, path)
print("R013_LOCAL_POLICY_WRITTEN")
PY
product_check 8 && echo R013_PRODUCT_CHECK_PASS || rollback_now PRODUCT_CHECK
start_and_confirm || rollback_now DASHBOARD_NOT_CONNECTED
echo R013_DASHBOARD_CONNECTED
# Webcam 006 binds the same Host and Studio; Runtime 013 must leave it valid.
python3 host/webcam_studio.py policy-check >/dev/null && echo R013_WEBCAM_006_POLICY_CHECK_PASS || rollback_now WEBCAM_POLICY_CHECK
echo "R013_CUTOVER_PASS main=$(git rev-parse --short HEAD) host=${HOST_EXPECTED:0:12} rollback=$RB"
