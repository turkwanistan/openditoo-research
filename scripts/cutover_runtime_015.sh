#!/usr/bin/env bash
# Runtime 015 cutover: Runtime 014 with the Slots v2 aim tuning only (host/slots_page.py).
# Same exact Host, target, webcam policy, session envelope, pacing, reconnect/reclaim and ButtonProbe.
#   bash scripts/cutover_runtime_015.sh "Grant OPENDITOO-PRODUCT-RUNTIME-015"
#   bash scripts/cutover_runtime_015.sh --rollback     # exact source tree + local policy back: Runtime 014
# Run it from the feat/slots-v2-aim worktree. It acts on the MAIN checkout (where the
...
[[ -n "$MAIN" && -d "$MAIN" ]] || { echo R015_MAIN_NOT_FOUND >&2; exit 2; }
cd "$MAIN"
LOCAL=.openditoo-local; POLICY=$LOCAL/product-runtime-policy.json; RB=$LOCAL/rollback-runtime-014
...
probe_ok(){
  python3 - "$HERE/product/OPENDITOO-PRODUCT-RUNTIME-015.json" <<'PY'
...
template_ok(){
  ...
  # Validate the exact Runtime 015 source and policy structure here; installed Host/Probe
...
raw = json.loads(v9.TEMPLATE.read_text(encoding="utf-8"))
v9.load_policy(v9.TEMPLATE, require_authority=False, verify_hashes=False)
assert raw["runtime_revision"] == 10
assert raw["build"]["code_sha256"] == v9.code_hashes()
assert v9.authority_blockers(v9.TEMPLATE) == [
...
stop_service(){
  ...
  echo R015_SERVICE_STOP_FAILED >&2; return 1
}
start_and_confirm(){
  ...
  echo "R015_DASHBOARD_NOT_CONFIRMED $r" >&2; return 1
}
...
if [[ "${1:-}" == "--rollback" ]]; then
  [[ -f "$RB/pre_commit" && -f "$RB/product-runtime-policy.json" ]] || { echo R015_NO_ROLLBACK_SAVED >&2; exit 2; }
  stop_service
  pre="$(cat "$RB/pre_commit")"
  # The merged range may contain merge commits, which `git revert A..B` cannot revert. Restore the
  # exact Runtime 014 tree as one forward commit instead (no history rewrite), then prove it.
  if [[ "$(git rev-parse 'HEAD^{tree}')" != "$(git rev-parse "$pre^{tree}")" ]]; then
    git read-tree -u --reset "$pre"
    git commit -q -m "Rollback: restore the Runtime 014 source tree ($(git rev-parse --short "$pre"))"
  fi
  [[ "$(git rev-parse 'HEAD^{tree}')" == "$(git rev-parse "$pre^{tree}")" ]] || { echo R015_ROLLBACK_TREE_MISMATCH >&2; exit 2; }
  install -m 600 "$RB/product-runtime-policy.json" "$POLICY"
  host_ok || { echo R015_ROLLBACK_HOST_HASH >&2; exit 2; }
  product_check 9 && echo R015_ROLLBACK_PRODUCT_CHECK_PASS
  start_and_confirm && echo R015_ROLLED_BACK_TO_RUNTIME_014
  exit 0
fi

[[ "${1:-}" == "Grant OPENDITOO-PRODUCT-RUNTIME-015" ]] || { echo R015_EXACT_GRANT_REQUIRED >&2; exit 2; }
[[ -z "$(git status --porcelain --untracked-files=no)" ]] || { echo R015_MAIN_HAS_TRACKED_CHANGES >&2; exit 2; }
git merge-base --is-ancestor HEAD "$BRANCH" || { echo R015_BRANCH_NOT_FAST_FORWARD >&2; exit 2; }
[[ "$(policy_id "$POLICY")" == OPENDITOO-PRODUCT-RUNTIME-014 ]] || { echo R015_CURRENT_POLICY_IS_NOT_014 >&2; exit 2; }
host_ok || { echo R015_INSTALLED_HOST_NOT_RUNTIME_014 >&2; exit 2; }
probe_ok || { echo R015_INSTALLED_BUTTON_PROBE_NOT_RUNTIME_014 >&2; exit 2; }
...
template_ok || { echo R015_TEMPLATE_NOT_HASH_VALID >&2; exit 2; }
echo R015_TEMPLATE_AND_INSTALLED_BINARIES_PASS
...
cp -p "$POLICY" "$RB/"
echo "R015_ROLLBACK_SAVED pre=$(cut -c1-12 "$RB/pre_commit")"

stop_service
python3 -c 'from host import webcam_trial; webcam_trial._host_preclaim_status()' \
  || { echo R015_HOST_NOT_IDLE >&2; systemctl --user start "$SERVICE"; exit 2; }
echo R015_HOST_IDLE
rollback_now(){ echo "R015_FAILED: $1 -- rolling back" >&2; bash "$HERE/scripts/cutover_runtime_015.sh" --rollback; exit 2; }

git merge --ff-only "$BRANCH" >/dev/null && echo "R015_MAIN_FAST_FORWARDED $(git rev-parse --short HEAD)"
...
echo R015_MAIN_OFFLINE_PASS

python3 - "$1" <<'PY' || rollback_now LOCAL_POLICY
import json, os, sys
from pathlib import Path
doc = json.loads(Path("product/OPENDITOO-PRODUCT-RUNTIME-015.json").read_text(encoding="utf-8"))
a = doc["authority"]
...
os.replace(tmp, path)
print("R015_LOCAL_POLICY_WRITTEN")
PY
product_check 10 && echo R015_PRODUCT_CHECK_PASS || rollback_now PRODUCT_CHECK
start_and_confirm || rollback_now DASHBOARD_NOT_CONNECTED
echo R015_DASHBOARD_CONNECTED
# Webcam 006 binds the same Host and Studio; Runtime 015 must leave it valid.
python3 host/webcam_studio.py policy-check >/dev/null && echo R015_WEBCAM_006_POLICY_CHECK_PASS || rollback_now WEBCAM_POLICY_CHECK
echo "R015_CUTOVER_PASS main=$(git rev-parse --short HEAD) host=${HOST_EXPECTED:0:12} rollback=$RB"
