# OPENDITOO-MASSBOOT-ENUM-001
# Enumeration only. No custom USB/SCSI/vendor command; no disk/volume access; no firmware/RAM read/write.
[CmdletBinding()]
param()
$ErrorActionPreference = 'Stop'

$exp = 'OPENDITOO-MASSBOOT-ENUM-001'
$root = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$outDir = Join-Path $root 'captures/private/massboot-enum-001'
...
Write-Host 'Make sure the SD card is REMOVED.' -ForegroundColor Yellow
Write-Host ''
Write-Host 'PHASE A — normal USB baseline'
Write-Host '1) Ditoo fully OFF.'
Write-Host '2) Connect the known data-capable USB cable to the PC/Ditoo.'
...
try {
  ...
  Write-Host '2) While holding it, attach the same data-capable USB cable.' -ForegroundColor Yellow
