#!/usr/bin/env bash
# Runtime 010 cutover: Runtime 009 plus Pocket Moss and generic modal navigation.
# Same exact Host, target, webcam policy, session envelope, pacing, reconnect/reclaim and ButtonProbe.
#   bash scripts/cutover_runtime_010.sh "Grant OPENDITOO-PRODUCT-RUNTIME-010"
#   bash scripts/cutover_runtime_010.sh --rollback     # exact source tree + local policy back: Runtime 009
# Run it from the feat/pocket-moss-pixel-lab worktree. It acts on the MAIN checkout (where the
...
set -euo pipefail
BRANCH=feat/pocket-moss-pixel-lab
...
[[ -n "$MAIN" && -d "$MAIN" ]] || { echo R010_MAIN_NOT_FOUND >&2; exit 2; }
cd "$MAIN"
LOCAL=.openditoo-local; POLICY=$LOCAL/product-runtime-policy.json; RB=$LOCAL/rollback-runtime-009
...
stop_service(){
  ...
  echo R010_SERVICE_STOP_FAILED >&2; return 1
}
start_and_confirm(){
  ...
  echo "R010_DASHBOARD_NOT_CONFIRMED $r" >&2; return 1
}
...
if [[ "${1:-}" == "--rollback" ]]; then
  [[ -f "$RB/pre_commit" && -f "$RB/product-runtime-policy.json" ]] || { echo R010_NO_ROLLBACK_SAVED >&2; exit 2; }
  stop_service
  pre="$(cat "$RB/pre_commit")"
  # The merged range may contain merge commits, which `git revert A..B` cannot revert. Restore the
  # exact Runtime 009 tree as one forward commit instead (no history rewrite), then prove it.
  if [[ "$(git rev-parse 'HEAD^{tree}')" != "$(git rev-parse "$pre^{tree}")" ]]; then
    git read-tree -u --reset "$pre"
    git commit -q -m "Rollback: restore the Runtime 009 source tree ($(git rev-parse --short "$pre"))"
  fi
  [[ "$(git rev-parse 'HEAD^{tree}')" == "$(git rev-parse "$pre^{tree}")" ]] || { echo R010_ROLLBACK_TREE_MISMATCH >&2; exit 2; }
  install -m 600 "$RB/product-runtime-policy.json" "$POLICY"
  host_ok || { echo R010_ROLLBACK_HOST_HASH >&2; exit 2; }
  product_check 4 && echo R010_ROLLBACK_PRODUCT_CHECK_PASS
  start_and_confirm && echo R010_ROLLED_BACK_TO_RUNTIME_009
  exit 0
fi

[[ "${1:-}" == "Grant OPENDITOO-PRODUCT-RUNTIME-010" ]] || { echo R010_EXACT_GRANT_REQUIRED >&2; exit 2; }
[[ -z "$(git status --porcelain --untracked-files=no)" ]] || { echo R010_MAIN_HAS_TRACKED_CHANGES >&2; exit 2; }
...
cp -p "$POLICY" "$RB/"
echo "R010_ROLLBACK_SAVED pre=$(cut -c1-12 "$RB/pre_commit")"

stop_service
python3 -c 'from host import webcam_trial; webcam_trial._host_preclaim_status()' \
  || { echo R010_HOST_NOT_IDLE >&2; systemctl --user start "$SERVICE"; exit 2; }
echo R010_HOST_IDLE
rollback_now(){ echo "R010_FAILED: $1 -- rolling back" >&2; bash "$HERE/scripts/cutover_runtime_010.sh" --rollback; exit 2; }

git merge --ff-only "$BRANCH" >/dev/null && echo "R010_MAIN_FAST_FORWARDED $(git rev-parse --short HEAD)"
...
echo R010_MAIN_OFFLINE_PASS

python3 - "$1" <<'PY' || rollback_now LOCAL_POLICY
import json, os, sys
from pathlib import Path
doc = json.loads(Path("product/OPENDITOO-PRODUCT-RUNTIME-010.json").read_text(encoding="utf-8"))
a = doc["authority"]
...
os.replace(tmp, path)
print("R010_LOCAL_POLICY_WRITTEN")
PY
product_check 5 && echo R010_PRODUCT_CHECK_PASS || rollback_now PRODUCT_CHECK
start_and_confirm || rollback_now DASHBOARD_NOT_CONNECTED
echo R010_DASHBOARD_CONNECTED
# Webcam 006 binds the same Host and Studio; Runtime 010 must leave it valid.
python3 host/webcam_studio.py policy-check >/dev/null && echo R010_WEBCAM_006_POLICY_CHECK_PASS || rollback_now WEBCAM_POLICY_CHECK
echo "R010_CUTOVER_PASS main=$(git rev-parse --short HEAD) host=${HOST_EXPECTED:0:12} rollback=$RB"
