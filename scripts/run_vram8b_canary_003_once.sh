...
cd "$ROOT"
EXPERIMENT_ID=OPENDITOO-VRAM8B-CANARY-003
GRANT_TEXT='Grant OPENDITOO-VRAM8B-CANARY-003 -- stock btplayer selected'
MANIFEST="experiments/${EXPERIMENT_ID}.json"
FIXTURE_REPORT=artifacts/analysis/volatile_ram_api_vram8b_fixture_003.json
EXPECTED_MANIFEST_SHA=adcf33e0883bbb2885715f8a4a86a23c0fc101ca4b6304c0e5f20e2366ef03a0
EXPECTED_FIXTURE_SHA=ebdb4be66725c15435d7ed0d24466ba06fb53b2225005d6e2cd13d25cf82161a
...
LOCAL_DIR=.openditoo-local/vram8b-canary-003
...
RUNNER_PROJECT=runtime/windows/OpenDitoo.Vram8B.Runner003/OpenDitoo.Vram8B.Runner003.csproj
RUNNER_DLL=runtime/windows/OpenDitoo.Vram8B.Runner003/bin/Release/net8.0/OpenDitoo.Vram8B.Runner003.dll
PS1=runtime/windows/run_openditoo_vram8b_canary_003_once.ps1
...
python3 tools/ditoo_vram8b_live_gate_003.py --selfcheck >/dev/null
...
files=['runtime/windows/OpenDitoo.Vram8B.Runner003/OpenDitoo.Vram8B.Runner003.csproj','runtime/windows/OpenDitoo.Vram8B.Runner003/FrozenVram8BProtocol.cs','runtime/windows/OpenDitoo.Vram8B.Runner003/WindowsRfcommVram8BTransport.cs','runtime/windows/OpenDitoo.Vram8B.Runner003/Program.cs','runtime/windows/run_openditoo_vram8b_canary_003_once.ps1','scripts/run_vram8b_canary_003_once.sh']
sha=lambda x: hashlib.sha256(open(x,'rb').read()).hexdigest()
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-003','manifest_sha256':'adcf33e0883bbb2885715f8a4a86a23c0fc101ca4b6304c0e5f20e2366ef03a0','fixture_report_sha256':'ebdb4be66725c15435d7ed0d24466ba06fb53b2225005d6e2cd13d25cf82161a','one_use_nonce':nonce,'assembly_sha256':sha(dll),'source_files':[{'path':x,'sha256':sha(x)} for x in files],'sealed_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
...
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-003','manifest_sha256':mh,'one_use_nonce':nonce,'runtime018_service_inactive':True,'created_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
tmp=p+'.tmp'; open(tmp,'w',encoding='utf-8').write(json.dumps(d,indent=2,sort_keys=True)+'\n'); os.replace(tmp,p)
PY
