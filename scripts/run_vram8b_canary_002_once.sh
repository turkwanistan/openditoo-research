...
cd "$ROOT"
EXPERIMENT_ID=OPENDITOO-VRAM8B-CANARY-002
GRANT_TEXT='Grant OPENDITOO-VRAM8B-CANARY-002 -- stock btplayer selected'
MANIFEST="experiments/${EXPERIMENT_ID}.json"
FIXTURE_REPORT=artifacts/analysis/volatile_ram_api_vram8b_fixture_002.json
EXPECTED_MANIFEST_SHA=0f054872b6886a6eacea9ae71ab43f01ef393d1cf55e4e242535f73d6a1cc758
EXPECTED_FIXTURE_SHA=6b87fe811a22614654bcb0fb30fd8ffcaa84442ef73291a7af4925920dc0fd9a
...
LOCAL_DIR=.openditoo-local/vram8b-canary-002
...
RUNNER_PROJECT=runtime/windows/OpenDitoo.Vram8B.Runner002/OpenDitoo.Vram8B.Runner002.csproj
RUNNER_DLL=runtime/windows/OpenDitoo.Vram8B.Runner002/bin/Release/net8.0/OpenDitoo.Vram8B.Runner002.dll
PS1=runtime/windows/run_openditoo_vram8b_canary_002_once.ps1
...
python3 tools/ditoo_vram8b_live_gate_002.py --selfcheck >/dev/null
...
files=['runtime/windows/OpenDitoo.Vram8B.Runner002/OpenDitoo.Vram8B.Runner002.csproj','runtime/windows/OpenDitoo.Vram8B.Runner002/FrozenVram8BProtocol.cs','runtime/windows/OpenDitoo.Vram8B.Runner002/WindowsRfcommVram8BTransport.cs','runtime/windows/OpenDitoo.Vram8B.Runner002/Program.cs','runtime/windows/run_openditoo_vram8b_canary_002_once.ps1','scripts/run_vram8b_canary_002_once.sh']
sha=lambda x: hashlib.sha256(open(x,'rb').read()).hexdigest()
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-002','manifest_sha256':'0f054872b6886a6eacea9ae71ab43f01ef393d1cf55e4e242535f73d6a1cc758','fixture_report_sha256':'6b87fe811a22614654bcb0fb30fd8ffcaa84442ef73291a7af4925920dc0fd9a','one_use_nonce':nonce,'assembly_sha256':sha(dll),'source_files':[{'path':x,'sha256':sha(x)} for x in files],'sealed_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
...
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-002','manifest_sha256':mh,'one_use_nonce':nonce,'runtime018_service_inactive':True,'created_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
tmp=p+'.tmp'; open(tmp,'w',encoding='utf-8').write(json.dumps(d,indent=2,sort_keys=True)+'\n'); os.replace(tmp,p)
PY
