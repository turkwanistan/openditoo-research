...
$manifestPath = Join-Path $repo 'experiments\DAY1-WEBCAM-N980P-007.json'
...
$evidencePath = Join-Path $repo 'captures\OPENDITOO-WEBCAM-W9B-007-PREPARATION-2026-09-10.json'
function Require([bool]$Condition,[string]$Reason) { if (-not $Condition) { throw $Reason } }
Write-Output 'W9B_OPTICAL_PREP_BEGIN experiment_id=OPENDITOO-WEBCAM-N980P-007 device_io=false host_session_io=false claim_created=false'
$doc = Get-Content $manifestPath -Raw | ConvertFrom-Json
Require ($doc.experiment_id -eq 'OPENDITOO-WEBCAM-N980P-007') 'W9B_007_MANIFEST_ID_MISMATCH'
Require ($doc.authority.transmission_authorized -eq $false) 'W9B_007_PREP_REFUSES_PREAUTHORIZED_MANIFEST'
Require ($doc.authority.authorization_consumed -eq $false) 'W9B_007_PREP_REFUSES_CONSUMED_MANIFEST'
Require ($doc.stream.playback_interval_ms -eq 50) 'W9B_007_CLIENT_INTERVAL_DRIFT'
Require ($doc.session.min_frame_interval_ms -eq 40) 'W9B_007_HOST_FLOOR_DRIFT'
Require ($doc.session.handover_settle_seconds -eq 8) 'W9B_007_SETTLE_DRIFT'
Require ($doc.budgets.max_frames -eq 201 -and $doc.budgets.max_application_packets -eq 603 -and $doc.budgets.max_tx_bytes -eq 211854) 'W9B_007_BUDGET_DRIFT'
& dotnet build (Join-Path $project 'OpenDitoo.Webcam.Runner.csproj') -c Release --nologo
if ($LASTEXITCODE -ne 0) { throw 'W9B_007_RUNNER_BUILD_FAILED' }
if (Test-Path $StageDir) { Remove-Item $StageDir -Recurse -Force }
New-Item -ItemType Directory -Path $StageDir | Out-Null
Copy-Item (Join-Path $release '*') $StageDir -Recurse
$runner = Join-Path $StageDir 'OpenDitoo.Webcam.Runner.exe'
Require (Test-Path $runner) 'W9B_007_STAGED_RUNNER_MISSING'
$adapterLines = @(& $runner selftest); $adapterExit=$LASTEXITCODE
foreach ($line in $adapterLines) { Write-Output $line }
if ($adapterExit -ne 0) { throw 'W9B_007_ADAPTER_SELFTEST_FAILED' }
$adapterText=$adapterLines -join "`n"
Require ($adapterText -match 'ADAPTER_W8_40MS_ARRIVAL_JITTER_REPRO_PASS') 'W9B_007_40MS_FAILURE_REPRO_MISSING'
Require ($adapterText -match 'ADAPTER_W8_50MS_ARRIVAL_JITTER_PASS') 'W9B_007_50MS_MARGIN_PASS_MISSING'
Require ($adapterText -match 'ADAPTER_HOST_ELAPSED_CROSS_CLOCK_PASS') 'W9B_007_CROSS_CLOCK_TELEMETRY_PASS_MISSING'
Require ($adapterText -match 'ADAPTER_W9A_SOURCE_IDENTITY_PASS') 'W9B_007_SOURCE_IDENTITY_PASS_MISSING'
& $runner transform-selftest $transformFixture
if ($LASTEXITCODE -ne 0) { throw 'W9B_007_TRANSFORM_PARITY_FAILED' }
& $runner encoder-selftest $encoderFixture
if ($LASTEXITCODE -ne 0) { throw 'W9B_007_ENCODER_PARITY_FAILED' }
$statusLines=@(& $runner host-status-selftest $repo); $statusExit=$LASTEXITCODE
foreach ($line in $statusLines) { Write-Output $line }
if ($statusExit -ne 0) { throw 'W9B_007_LIVE_STATUS_SELFTEST_FAILED' }
$statusPass=$null
...
Require ($null -ne $statusPass) 'W9B_007_LIVE_STATUS_PASS_RECORD_MISSING'
Require ($statusPass.target -eq '11:75:58:CE:DE:C7') 'W9B_007_LIVE_STATUS_TARGET_MISMATCH'
Require ($statusPass.device_io -eq $false -and $statusPass.host_session_io -eq $false) 'W9B_007_STATUS_SELFTEST_IO_BOUNDARY_BROKEN'
foreach ($property in $doc.stream.live_source.producer_code_sha256.PSObject.Properties) {
    ...
    Require (Test-Path $sourcePath) ('W9B_007_PRODUCER_MISSING '+$relative)
...
Require (Test-Path $framingPath) 'W9B_007_FRAMING_EVIDENCE_MISSING'
$framing = Get-Content $framingPath -Raw | ConvertFrom-Json
Require ($framing.status -eq 'pass') 'W9B_007_FRAMING_NOT_PASS'
Require ($framing.monotonic -eq $true) 'W9B_007_FRAMING_SEQUENCE_NOT_MONOTONIC'
Require ($framing.operation.device_io -eq $false -and $framing.operation.claim_created -eq $false) 'W9B_007_FRAMING_IO_BOUNDARY_BROKEN'
$gitHeadLines=@(& wsl.exe -d $Distro -- git -C $WslRepositoryPath rev-parse HEAD)
if ($LASTEXITCODE -ne 0) { throw 'W9B_007_GIT_HEAD_LOOKUP_FAILED' }
$gitHead=($gitHeadLines|Select-Object -Last 1); Require (-not [string]::IsNullOrWhiteSpace($gitHead)) 'W9B_007_GIT_HEAD_EMPTY'; $gitHead=$gitHead.Trim()
$evidence=[ordered]@{
 schema_version=1; experiment_id='OPENDITOO-WEBCAM-N980P-007'; milestone='W9B-007 optical/identity trial preparation'; status='pass'; prepared_from_git_head=$gitHead
...
$doc.readiness.required_followup='Obtain the fresh exact grant `Grant OPENDITOO-WEBCAM-N980P-007`. Then execute only scripts/run_webcam_w9b_optical_007_windows.ps1 once, with the operator filming. Never replay 002-005 and never create a rate ladder.'
...
if ($LASTEXITCODE -ne 0) { throw 'W9B_007_WSL_OFFLINE_VALIDATION_FAILED_VERIFY' }
& wsl.exe -d $Distro -- python3 ($WslRepositoryPath + '/scripts/check_webcam_w9b_optical_007.py')
if ($LASTEXITCODE -ne 0) { throw 'W9B_007_WSL_OFFLINE_VALIDATION_FAILED_CHECK' }
Write-Output ("W9B_OPTICAL_PREP_PASS experiment_id=OPENDITOO-WEBCAM-N980P-007 manifest_sha256={0} evidence_sha256={1} claim_created=false host_session_io=false device_io=false" -f $manifestSha,$evidenceSha)
Write-Output 'W9B_OPTICAL_NEXT_GRANT=Grant OPENDITOO-WEBCAM-N980P-007'
