...
cd "$ROOT"
EXPERIMENT_ID=OPENDITOO-VRAM8B-CANARY-006
GRANT_TEXT='Grant OPENDITOO-VRAM8B-CANARY-006 -- stock btplayer selected'
MANIFEST="experiments/${EXPERIMENT_ID}.json"
FIXTURE_REPORT=artifacts/analysis/volatile_ram_api_vram8b_fixture_006.json
EXPECTED_MANIFEST_SHA=0cc58e980c0d73c61113adb6a3068bc706fa07bf9cc420259a7d348f554b5540
EXPECTED_FIXTURE_SHA=886f99add1c3e4128c2923509ff58e3d451e5a89cf4877ee690cc03b438bb9f6
...
LOCAL_DIR=.openditoo-local/vram8b-canary-006
...
RUNNER_PROJECT=runtime/windows/OpenDitoo.Vram8B.Runner006/OpenDitoo.Vram8B.Runner006.csproj
RUNNER_DLL=runtime/windows/OpenDitoo.Vram8B.Runner006/bin/Release/net8.0/OpenDitoo.Vram8B.Runner006.dll
PS1=runtime/windows/run_openditoo_vram8b_canary_006_once.ps1
...
python3 tools/ditoo_vram8b_live_gate_006.py --selfcheck >/dev/null
...
files=['runtime/windows/OpenDitoo.Vram8B.Runner006/OpenDitoo.Vram8B.Runner006.csproj','runtime/windows/OpenDitoo.Vram8B.Runner006/FrozenVram8BProtocol.cs','runtime/windows/OpenDitoo.Vram8B.Runner006/WindowsRfcommVram8BTransport.cs','runtime/windows/OpenDitoo.Vram8B.Runner006/Program.cs','runtime/windows/run_openditoo_vram8b_canary_006_once.ps1','scripts/run_vram8b_canary_006_once.sh']
sha=lambda x: hashlib.sha256(open(x,'rb').read()).hexdigest()
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-006','manifest_sha256':'0cc58e980c0d73c61113adb6a3068bc706fa07bf9cc420259a7d348f554b5540','fixture_report_sha256':'886f99add1c3e4128c2923509ff58e3d451e5a89cf4877ee690cc03b438bb9f6','one_use_nonce':nonce,'assembly_sha256':sha(dll),'source_files':[{'path':x,'sha256':sha(x)} for x in files],'sealed_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
...
d={'schema_version':1,'experiment_id':'OPENDITOO-VRAM8B-CANARY-006','manifest_sha256':mh,'one_use_nonce':nonce,'runtime018_service_inactive':True,'created_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
tmp=p+'.tmp'; open(tmp,'w',encoding='utf-8').write(json.dumps(d,indent=2,sort_keys=True)+'\n'); os.replace(tmp,p)
PY
