...
transmit bytes.  It freezes one exact sequence for OPENDITOO-VRAM67-BXLR-002 and
fails closed against the four authoritative Tier-2/VRAM artifacts and the consumed 001 transport-harness result.
"""
...
PREDECESSOR_RESULT = ROOT / "captures/OPENDITOO-VRAM67-BXLR-001-LIVE-RESULT-2026-09-12.json"
...
FIXTURE_REPORT = ROOT / "artifacts/analysis/volatile_ram_api_vram67_fixture_002.json"
...
def _verify_predecessor() -> dict[str, Any]:
    d = _json(PREDECESSOR_RESULT)
    _require(d["experiment_id"] == "OPENDITOO-VRAM67-BXLR-001", "predecessor id drift")
    _require(d["status"] == "consumed_inconclusive_transport_harness", "predecessor classification drift")
    _require(d["authority"]["consumed"] is True, "predecessor authority drift")
    _require(d["wire_progress"]["application_send_calls_completed"] == 1, "predecessor send-count drift")
    _require(d["wire_progress"]["voicetip_0xa5"]["send_call_reached"] is False, "predecessor 0xa5 progress drift")
    _require(d["wire_progress"]["custom_overwrite_0x6c"]["transmitted"] is False, "predecessor custom overwrite drift")
    _require(d["interpretation"]["classification"] == "TRANSPORT_HARNESS_INCONCLUSIVE_NOT_VRAM_NEGATIVE", "predecessor interpretation drift")
    return {
        "experiment_id": d["experiment_id"],
        "path": str(PREDECESSOR_RESULT.relative_to(ROOT)),
        "sha256": _sha_path(PREDECESSOR_RESULT),
        "classification": d["interpretation"]["classification"],
        "custom_0x6c_transmitted": False,
        "same_grant_replay_allowed": False,
    }
...
def build_report() -> dict[str, Any]:
    ...
    return {
        ...
        "transport_correction": {
            "reason": "001 incorrectly waited for a fresh FD_WRITE before each send and timed out before the 0xa5 send call",
            "successor_rule": "wait for initial FD_WRITE once; thereafter exactly one direct send() call per frozen application frame; WSAEWOULDBLOCK/short/error is terminal NO_RETRY",
