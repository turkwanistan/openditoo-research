...
V3_ROOT = Path("assets/pocket_moss/v3")

class MossV3ArtTests(unittest.TestCase):
    def test_v3_idle_keeps_dog_cues_with_depth_plane(self):
        path=V3_ROOT/'sprites'/'idle.json'; raw=json.loads(path.read_text()); sprite=load_sprite(path)
    ...
    def test_active_v3_animations_are_warning_free_and_materially_richer(self):
        ...
        for name,min_unique in minimum_unique.items():
            with self.subTest(name=name):
                a=load_animation(V3_ROOT/'animations'/f'{name}.json'); q=animation_quality(a)
    ...
    def test_dance_contains_anticipation_hop_landing_and_wag_phases(self):
        a=load_animation(V3_ROOT/'animations'/'dance.json')
