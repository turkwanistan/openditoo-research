...
from host import product_runtime_v4  # noqa: E402
from host import product_runtime_v5  # noqa: E402
from host import product_runtime_v6  # noqa: E402
from host import product_runtime_v7  # noqa: E402
from host import product_runtime_v8  # noqa: E402
...
CURRENT_PRODUCT_TEMPLATE = ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-016.json"
...
class ProductRuntimeV2Tests(unittest.TestCase):
    ...
    def test_the_current_committed_policy_is_disabled_and_hash_complete(self) -> None:
        ...
        reviewed = product_runtime_v10.load_policy(path, require_authority=False, verify_hashes=False)
        self.assertEqual(reviewed.product_id, product_runtime_v2.PRODUCT_ID)
        self.assertEqual(raw["build"]["code_sha256"], product_runtime_v10.code_hashes())
        ...
        self.assertIn("PRODUCT_AUTHORITY_MISSING", product_runtime_v10.authority_blockers(path))
        with self.assertRaises(product_runtime_v2.ProductPolicyError) as blocked:
            product_runtime_v10.load_policy(path, require_authority=True, verify_hashes=False)
        self.assertEqual(blocked.exception.code, "PRODUCT_AUTHORITY_MISSING")
...
class ProductInstallationBoundaryTests(unittest.TestCase):
    ...
    def test_the_current_committed_template_is_disabled_and_hash_complete(self) -> None:
        path = CURRENT_PRODUCT_TEMPLATE
        raw = json.loads(path.read_text(encoding="utf-8"))
        self.assertEqual(raw["runtime_revision"], 11)
        self.assertFalse(raw["authority"]["persistent_runtime_authorized"])
        self.assertIsNone(raw["authority"]["grant_scope"])
        reviewed = product_runtime_v10.load_policy(path, require_authority=False, verify_hashes=False)
        self.assertEqual(raw["build"]["code_sha256"], product_runtime_v10.code_hashes())
...
class W6WebcamReviewTests(unittest.TestCase):
    ...
    def test_review_envelope_is_valid_bounded_and_grant_ready(self) -> None:
        ...
        self.assertEqual(doc["session"]["min_frame_interval_ms"], 40)
        self.assertEqual(stream["playback_interval_ms"], 90)
        self.assertEqual(doc["budgets"]["max_frames"], 10_000 // 90 + 1)
        self.assertEqual(doc["budgets"]["max_application_packets"], 336)
        self.assertEqual(doc["budgets"]["max_tx_bytes"], 112 * frame_stream.worst_case_frame_tx_bytes())
        readiness = doc["readiness"]
        self.assertFalse(readiness["execution_ready"])
        if doc["status"] == "completed_pass_authority_consumed":
        ...
        verification = doc["w6_verification"]
    ...
    def test_completed_w7_authority_is_consumed_and_not_replayable(self) -> None:
        doc = json.loads(self.MANIFEST.read_text(encoding="utf-8"))
        self.assertEqual(doc["experiment_id"], "OPENDITOO-WEBCAM-N980P-002")
        ...
        self.assertFalse(doc["readiness"]["grant_ready"])
        self.assertEqual(doc["readiness"]["blockers"], ["AUTHORITY_ALREADY_CONSUMED"])
...
class W8WebcamNearCeilingReviewTests(unittest.TestCase):
    ...
    def test_w8_003_preserves_w7_baseline_but_uses_distinct_consumed_authority(self) -> None:
        ...
        self.assertTrue(w7["authority"]["authorization_consumed"])
        self.assertEqual(w7["w7_attempt"]["owner_visible_acceptance"], "pass")
    ...
    def test_w8_runner_paces_from_manifest_and_measures_the_new_metrics(self) -> None:
        ...
        self.assertIn("MaxDepthObserved", slot)

    def test_w8_prep_is_offline_only_and_cannot_grant_itself(self) -> None:
        script = (ROOT / "scripts/prepare_webcam_w8_windows.ps1").read_text(encoding="utf-8")
        for required in ("$runner selftest", "$runner transform-selftest", "$runner encoder-selftest",
                         "$runner host-status-selftest", "ADAPTER_W8_ACK_FLOOR_PASS", "W8_PREP_PASS",
                         "claim_created=false", "host_session_io=false", "device_io=false",
        ...
        for forbidden in ("cli/webcam.py run", "systemctl --user stop",
            ...
            self.assertNotIn(forbidden, script)
        self.assertIn("W8_PREP_REFUSES_PREAUTHORIZED_MANIFEST", script)
        self.assertIn("W8_PREP_REFUSES_CONSUMED_MANIFEST", script)

    def test_w8_launcher_is_separate_one_controller_one_use_path(self) -> None:
        shell = (ROOT / "scripts/run_webcam_w8_once.sh").read_text(encoding="utf-8")
        windows = (ROOT / "scripts/run_webcam_w8_windows.ps1").read_text(encoding="utf-8")
        self.assertIn('MANIFEST="experiments/DAY1-WEBCAM-N980P-003.json"', shell)
        self.assertIn("max_frames=251 max_packets=753 max_tx_bytes=264554", shell)
        self.assertIn("python3 scripts/check_webcam_w8.py", shell)
        self.assertIn('systemctl --user stop "$SERVICE"', shell)
        self.assertIn('python3 cli/webcam.py run --manifest "$MANIFEST"', shell)
        self.assertIn("W8_RESTORE_PRODUCT_CONNECTED_PASS", shell)
        self.assertIn("W8_DUPLICATE_SOURCE_FRAME", shell)
        self.assertIn("W8_QUEUE_DEPTH_BROKEN", shell)
        self.assertIn("rate_target_met=", shell)
...
class PaginationTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.events_path = self.dir / "button-events.ndjson"
        self.clock = {"s": 100.0}
    ...
    def _frames(self) -> frame_stream.FrameSet:
        return frame_stream.build_frame_set(frame_stream.frames_from_source(ROOT / "assets/pagination/spiral-10.rgb888"))
    ...
    class Dashboard:
        def __init__(self) -> None:
            ...
            self.pulse_sources: set = set()

        def __call__(self, _now):
            self.calls += 1
            return bytes([0, 200, 0]) * 256, self.pulse_step is not None
    ...
    def test_016_template_hashes_match_this_tree_and_older_revisions_are_superseded_records(self) -> None:
        path = ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-016.json"
        raw = json.loads(path.read_text(encoding="utf-8"))
        product_runtime_v10.load_policy(path, require_authority=False, verify_hashes=False)
        self.assertEqual(raw["build"]["code_sha256"], product_runtime_v10.code_hashes())
        old015 = json.loads((ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-015.json").read_text(encoding="utf-8"))
        self.assertNotEqual(old015["build"]["code_sha256"], product_runtime_v9.code_hashes())
        old014 = json.loads((ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-014.json").read_text(encoding="utf-8"))
        self.assertNotEqual(old014["build"]["code_sha256"], product_runtime_v8.code_hashes())
        old013 = json.loads((ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-013.json").read_text(encoding="utf-8"))
        self.assertNotEqual(old013["build"]["code_sha256"], product_runtime_v7.code_hashes())
        old012 = json.loads((ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-012.json").read_text(encoding="utf-8"))
        self.assertNotEqual(old012["build"]["code_sha256"], product_runtime_v6.code_hashes())
        old011 = json.loads((ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-011.json").read_text(encoding="utf-8"))
        self.assertNotEqual(old011["build"]["code_sha256"], product_runtime_v5.code_hashes())
        old010 = json.loads((ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-010.json").read_text(encoding="utf-8"))
        self.assertNotEqual(old010["build"]["code_sha256"], product_runtime_v4.code_hashes())
        old009 = json.loads((ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-009.json").read_text(encoding="utf-8"))
        self.assertNotEqual(old009["build"]["code_sha256"], product_runtime_v3.code_hashes())
        old008 = json.loads((ROOT / "product/OPENDITOO-PRODUCT-RUNTIME-008.json").read_text(encoding="utf-8"))
        self.assertNotEqual(old008["build"]["code_sha256"], product_runtime_v3.code_hashes())
        for name in ("OPENDITOO-PRODUCT-RUNTIME-007.json", "OPENDITOO-PRODUCT-RUNTIME-006.json"):
            old = json.loads((ROOT / "product" / name).read_text(encoding="utf-8"))
            self.assertNotEqual(old["build"]["code_sha256"], pagination.code_hashes())
