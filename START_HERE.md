...
- **Handoff: `notes/OPENDITOO-HANDOFF-2026-09-11-POCKET-MOSS-V4.md`** — Runtime 013 / Pocket Moss v4 is the accepted live baseline. v1 failed recognition (“brown blob”); v2 fixed identity; v3 added depth and richer actions; v4 is the art/animation/style pass (front-facing seated body, centred face, directional highlight/shadow, teal collar + gold tag, richer Look/Loaf/Sleep-Wake/Pet/Dance/Kisses). Owner verdict on the physical panel: **“i love it.”**
- **Live product: Runtime 013** (`runtime_revision=8`, policy `OPENDITOO-PRODUCT-RUNTIME-013`, main cutover `b14ef9f`): Dashboard + Slots + Moss in one `streaming_ack_clock` Host session; exact Ditoo target, Host `3faf520f…`, ButtonProbe, pacing, selector semantics, reconnect/reclaim behavior and **webcam policy 006** unchanged. Runtime 012 is the exact rollback in `.openditoo-local/rollback-runtime-012` (`bash scripts/cutover_runtime_013.sh --rollback`).
...
- **Verification/cutover:** interactive successor gate PASS 111 tests; Pixel Lab warnings zero; live cutover `R013_TEMPLATE_AND_INSTALLED_BINARIES_PASS`, `R013_MAIN_OFFLINE_PASS`, `R013_PRODUCT_CHECK_PASS`, `R013_DASHBOARD_CONNECTED`, `R013_WEBCAM_006_POLICY_CHECK_PASS`, `R013_CUTOVER_PASS`; post-cutover `product-status` connected, `last_error=null`.
