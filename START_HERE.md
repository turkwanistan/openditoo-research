...
- **ACTIVE SOFTWARE RESEARCH: `notes/OPENDITOO-HANDOFF-2026-09-12-VOLATILE-RAM-API.md` + `notes/OPENDITOO-VOLATILE-RAM-API-PARSER-PLAN-2026-09-12.md`** — the no-hardware volatile-RAM route has completed **VRAM-0/VRAM-1 and Tier-1 VRAM-2 direct-SPP exhaustion**. `tools/ditoo_spp_surface.py` / `artifacts/analysis/volatile_ram_api_surface.json` account for 251/251 top-level slots, 9/9 implemented EXTERN subcommands and 124/124 explicit non-default handlers with **0 unresolved**. No direct caller-controlled RAM-write or control-flow primitive was promoted; no live manifest is justified. Next named tier is the remotely reachable **external media/codec sink behind `0x6c -> 0x34144 -> fixed fallback 0xA6490 / registered media callback`**, where reachability must be proven before generic codec work. Runtime 018 remains untouched. No live malformed/custom parser traffic without a new one-use manifest/grant. MassBoot/SPI measurement work remains deferred; M2 is authorized/unconsumed but must not be executed merely because authority exists.
...
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 364 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
