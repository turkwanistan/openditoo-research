...
- **Live product: Runtime 017** (`runtime_revision=12`, `host/product_runtime_v11.py`, policy `OPENDITOO-PRODUCT-RUNTIME-017`, main cutover `76b65be`, 2026-09-12). This is Runtime 016's pages, Host `3faf520f…`, session, pacing, reconnect/reclaim and webcam policy 006, with input replaced by media-resilient raw AVRCP (see the Runtime 017 section below). Runtime 016 is the exact rollback in `.openditoo-local/rollback-runtime-016` (`bash scripts/cutover_runtime_017.sh --rollback`); earlier rollbacks are kept.
