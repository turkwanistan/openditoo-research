...
- **ACTIVE SOFTWARE RESEARCH: `notes/OPENDITOO-HANDOFF-2026-09-12-VOLATILE-RAM-API.md` + `notes/OPENDITOO-VOLATILE-RAM-API-PARSER-PLAN-2026-09-12.md`** — Tier-2 now closes the full chain **offline across 4/4 preserved Plus branches**, with one important sequencing correction: `0x6c` does **not** prime its own content mode. Stock `0x6e SPP_DRAWING_CTRL_MOVIE_PLAY` with `payload[1] != 0` first selects content mode `0x0b` through the common dispatcher and immediately confirms that mode; only then does `0x6c SPP_DRAWING_ENCODE_MOVIE_PLAY` take the same-mode display path. The resulting RAM-only `CALLER_CONTROLLED_ADJACENT_HEAP_OVERWRITE` still provides 2041 controlled source bytes into 1032 bytes of physical destination capacity, max 1009 bytes beyond the allocation. Stage-1 bootstrap evidence proves the cold-start geometry: display backing `0x00804470`, controlled destination `0x00804778`, persistent VoiceTip/runtime50 `0x00804b80`, callback `+0x34 = 0x00804bb4`, callback source offset `0x43c`. The mode-`0x0b` initializer may allocate a separate later 0xC50 light-word object but has no direct app-heap free edge; the vulnerable display backing/runtime50 allocations remain persistent, so the prime cannot move the proven adjacency. Under a stock/manual btplayer precondition, `0x6e(nonzero)` primes content mode `0x0b`, ordinary `0xa5 SPP_SET_ALARM_LISTEN` selector 2/model `0x22` creates the VoiceTip state, and an **exact 1088-byte** `0x6c` source controls runtime50 byte0=`0xff`, byte8=`0x22`, and callback=`0x00804779` while preserving `+0x3c/+0x40/+0x44`. The period-1 worker's timeout path eventually reaches the callback; `0xbfe8` is only the transient app-heap busy flag and both malloc/free wrappers clear it before return. VRAM-3 proves `0x00804779` is executable Thumb RAM with no NX barrier, and the minimal offline stage-0 witness is two-byte `BX LR` (`70 47`) in a previously `0xff`-filled, unreferenced span. Therefore **deterministic victim placement, deterministic post-overwrite callback invocation, and a clean returning controlled indirect branch are promoted offline**. The all-SPP `0x8a` BLUE-mode normalization remains unpromoted because queue/preemption ordering is not statically closed; use stock/manual btplayer as the deterministic precondition. **VRAM-6/7 LIVE PASS:** `OPENDITOO-VRAM67-BXLR-001` remains a consumed transport-harness-inconclusive historical attempt, but fresh successor `OPENDITOO-VRAM67-BXLR-002` completed the precommitted discriminator on the purchased unit. One RFCOMM connection completed all three exact frozen sends (`0x6e`, stock `0xa5`, exactly one custom `0x6c`), then survived the full 75,009 ms post-overwrite observation window; accepted Runtime 018 restored to revision 13, `connected`, `last_error=null`, and ACKed frames in a fresh session. Under the already-promoted deterministic placement + VoiceTip timeout/callback model, this is the planned live returning Thumb `BX LR` PASS. There is intentionally no separate wire-level stage0 marker, so preserve that inference boundary. Durable evidence: `captures/OPENDITOO-VRAM67-BXLR-002-LIVE-RESULT-2026-09-12.json`. **Both 001 and 002 are consumed; do not replay either.** The adopted next-phase roadmap is `notes/OPENDITOO-VRAM8-11-RAM-API-ROADMAP-2026-09-12.md`. **VRAM-8A is now CLOSED OFFLINE**: `tools/ditoo_vram8_stage0_model.py` pins the callback ABI across 4/4 branches and freezes a 20-byte leaf Thumb canary at fresh first-execution entry `0x00804901` (SHA-256 `1cdd53f83a75021b47cedc65bbac9f5ee1d488863d029d5900d4687efcf5264b`). It preserves `r4`/SP/LR, makes no stock/helper call, and toggles exactly one volatile stock energy-control byte through `*(u32*)0x008030f4 + 0x0f`; stock `0xb3 SPP_GET_ENERGY_CTRL` provides the typed positive readback. `experiments/OPENDITOO-VRAM8B-CANARY-001.json` is the fresh hash-bound one-use **PREPARED / UNAUTHORIZED** 8B manifest: fixed stock baseline/query -> frozen prime/VoiceTip/one custom overwrite -> typed `1` readback -> stock restore/query `0`; no runner, grant, claim, Bluetooth open, Runtime 018 handover or transmission has been materialized. **The exact next live boundary is a fresh owner grant for VRAM-8B, not VRAM-8D or VRAM-9.** In parallel, VRAM-8C is `DESIGN_ADVANCED_NOT_CLOSED`: a deliberately retained app-heap allocation ranks first, while hardcoded free-looking heap, display backing, runtime50 and unproven upper SRAM are rejected; callback task/IRQ context, allocator ABI/reentrancy and measured stage-1 size remain blockers. Do not jump to 8D/9, a generic loader/API, or replay VRAM67-001/002; none of their authority transfers forward.
- **Active recovery route: `notes/OPENDITOO-SOFTWARE-FIRST-RECOVERY-R0-2026-09-12.md`** — the owner-requested software-first detour is now **evidence-complete at its planned stop condition**. SFR-1 closed stock SPP readback as `SPP_READBACK_NO_CALLER_CONTROLLED_CHAIN_WITHIN_ANALYZED_GRAPH`; SFR-3 found only current production/test OTA branches; SFR-4 showed the readback result survives the official test-branch lineage; and SFR-2 recovered a provenance-strong Divoom Android 3.1.58 APK, mapped its hidden test-version/test-server controls, and found no historical-version or recovery selector. The still-live period `apptest` host currently returns the same 42016/42017 and 60014/60016 objects as the modern service. **Exact v42012 remains unrecovered.** That recovery branch is closed at its planned stop condition; the owner has now chosen the separate volatile-RAM/parser route above rather than further physical recovery work. M1 remains historical/deferred and M2 is authorized/unconsumed but deferred by current owner preference.
- **MassBoot R2 physical route (deferred, preserved): `notes/OPENDITOO-HANDOFF-2026-09-12-MASSBOOT-R2.md`** — MB-S versus MB-H distinction and the prepared photos-only M1 boundary remain valid. Do not consume/re-arm/change `OPENDITOO-MASSBOOT-M1-PHOTOS-001`; it is simply not the immediate route.
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 381 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
...
- **Moss art workflow:** v4 is generated by `tools/moss_v4_art.py` (one shared head/ear/face/body/tail model); `tests/test_moss_v4_art.py` pins the committed JSON to its output. Future polish = edit the generator, emit a NEW `assets/pocket_moss/vN`, and ship it as a fresh runtime successor (clone the 013 shape); never rewrite a live, hash-bound asset root. Preserve the face vocabulary and interaction semantics.
- **Verification/cutover:** Runtime 016 interactive successor gate PASS, 136 tests. Worktree Day-1 failures matched the clean baseline (13, from git-ignored build outputs). Live cutover reported `R016_MAIN_OFFLINE_PASS`, `R016_PRODUCT_CHECK_PASS`, `R016_DASHBOARD_CONNECTED`, `R016_WEBCAM_006_POLICY_CHECK_PASS` and `R016_CUTOVER_PASS`; post-cutover `product-status` connected, `last_error=null`.
- **Successor recipe:** clone the previous runtime with the pattern in the Slots handoff: new `product_runtime_vN`, template, CLI route, cutover script, tests, the Day-1 current-template retarget, and the verifier PASS lines. Author cutover scripts with the Write tool. Develop in a `--relative-paths` worktree, never on `main`.
...
### Live input successor — Runtime 017 (2026-09-11/12)

Bluetooth-media contention is localized, and the receive-only successor lives on `feat/media-avrcp-input` in `.openditoo-local/worktrees/media-avrcp`. Raw AVRCP is the authoritative input (`0x4C` left, `0x4B` right, `0x44|0x46` lever, press only). Fixed-Playing SMTC is only the ownership sink. The broker learns the Ditoo ACL handle at runtime from OpenDitoo's own outbound image preambles, reads raw L2CAP with RFCOMM/AVCTP dissection disabled, drops ETW backlog and flushes BTVS's ETW session every 50 ms. **Receive-only active-media acceptance PASS** (2026-09-11 21:16Z; latency p50 104 ms, media reaction NO, Tivoo excluded, 0 orphans). The sidecar runs elevated only through the admin-only on-demand scheduled task `OpenDitoo Raw AVRCP Broker` (installed once from Administrator PowerShell; the WSL runtime stays unelevated and holds a lease). That path's acceptance also PASSED. Runtime 017 went live 2026-09-12 (owner check with media playing: working) and was superseded the same night by **Runtime 018** (hidden BTVS window + rebind renewal; see the handoff). Read `notes/OPENDITOO-HANDOFF-2026-09-11-R017-MEDIA-AVRCP.md` first, then `notes/OPENDITOO-MEDIA-CONTENTION-RAW-AVRCP-2026-09-11.md`.
...
| HF-4 standing successor | **LIVE as Runtime 009** (2026-09-11): Runtime 008 cut over 00:45Z, owner review → Runtime 009 (spark-free lightning glow, first-ACK telemetry) 00:55Z; standing acceptance PASS (transitions, lever reclaims, lightning ×3 sources, rollover, 2 power cycles, 2 webcam suspend/restores). Rollback chain 009 → 008 → 007 |
