...
| W8 near-ceiling webcam rate | **implementation ready for Windows prep**: fresh `OPENDITOO-WEBCAM-N980P-003`, 40 ms ACK-gated client floor, 251-frame ceiling, expanded freshness/transport telemetry; unauthorized |
...
**W8 is now implemented but not grant-ready yet.** Fresh identity `OPENDITOO-WEBCAM-N980P-003` preserves the W7 camera/transform/encoder/Host path and changes only the client pacing/telemetry: a prior ACK is mandatory and dispatch starts are held to the deployed Host's existing 40 ms streaming backstop, giving a 10 s hard ceiling of 251 frames / 753 packets / 264,554 bytes. W7's measured ACK p50 was 27.17 ms, so the older plan's proposed `minFrameIntervalMs=0` / immediate-after-ACK behavior is superseded: it could violate the live Host's 40 ms backstop. W8 also records quarter FPS, quarter source-age, dispatch jitter, Host send-to-ACK time, localhost/HTTP overhead estimate, camera replacement rate, duplicate timestamps, and capacity-one queue-depth maxima. Current offline suite is **249 tests PASS**. Run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/prepare_webcam_w8_windows.ps1`; this is no-device preparation and must pass before requesting `Grant OPENDITOO-WEBCAM-N980P-003`.
