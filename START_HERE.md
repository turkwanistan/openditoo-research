...
- **Handoff: `notes/OPENDITOO-HANDOFF-2026-09-11-POCKET-MOSS-V2.md`** — Runtime 010 is now live/working, but v1 Moss failed physical readability (brown blob). Runtime 011 is the offline/unauthorized art-only v2 successor on `feat/pocket-moss-v2-art`; exact grant `Grant OPENDITOO-PRODUCT-RUNTIME-011` is required before v2 physical acceptance.
...
- **Successor branch:** `feat/pocket-moss-v2-art` contains Runtime 011, the complete Moss v2 art vocabulary, readability regressions and guarded 011 cutover. The committed 011 template remains unauthorized; live Runtime 010/main is the rollback baseline until a fresh 011 grant/cutover.
