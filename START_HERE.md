...
| W9B optical unique-frame/latency trial | **offline tooling implemented**: both regions decode from one shared exposure so no clock alignment is needed; first-appearance correlation yields p50/p95/min/max scene→visible latency plus visible transitions, repeats, skips and undecodable counts; impossible negative matches are excluded, not averaged. Synthetic end-to-end recovered an injected 29.1667 ms exactly. Operator slow-mo available; physical setup, fresh manifest and new exact grant still required. See route §28 |
