...
- **Active recovery route: `notes/OPENDITOO-SOFTWARE-FIRST-RECOVERY-R0-2026-09-12.md`** — by owner preference, the prepared MassBoot M1 disassembly/photo session is deferred while software-only recovery avenues are exhausted. R0 now has a deterministic stock-SPP recovery-surface analyzer/artifact: `0x93..0x96` are exact-model default/error handlers, `0x98/0x99` are update ingress, EXTERN is narrow, and all three preserved Ditoo/Ditoo-Plus images have 63 direct raw-SPI-read callers with zero direct SPP-handler edges. A deeper pass then proved real **indirect** SPP->flash reads for fixed typed config/state selectors (`0x27`, `0x80`, `0x81`, `0x6e`, `0xba`), but no caller-selected flash address in those pinned shortest chains. Next priority is SFR-1 payload/data-flow reachability, then period-app/API archaeology. SD updater development stays parked until recovery/rollback is solved. M1 remains prepared/unauthorized and may be resumed later; no hardware action is currently requested.
...
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 351 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
