...
| W8 near-ceiling webcam rate | **003 consumed on pacing violation; 004 awaiting Windows prep**: equal 40 ms client/Host floors proved unsafe under variable request-arrival latency; fresh 004 uses 50 ms client floor / 40 ms Host backstop, 201-frame ceiling |
...
Fresh replacement `OPENDITOO-WEBCAM-N980P-004` keeps the Host at 40 ms but raises only the ACK-gated client dispatch-start floor to **50 ms**, leaving a 10 ms request-arrival margin and a 20 fps theoretical client ceiling. Its 10 s hard limits are **201 frames / 603 packets / 211,854 bytes**. Offline regression now requires the old 40 ms shape to reproduce a pacing refusal under an 8 ms simulated arrival-latency swing and the new 50 ms shape to remain clean under the same swing. Current suite is **254 tests PASS**. Candidate 004 is unauthorized and not yet grant-ready; run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/prepare_webcam_w8_rerun_windows.ps1`. Only after that no-device prep passes may the project request `Grant OPENDITOO-WEBCAM-N980P-004`.
