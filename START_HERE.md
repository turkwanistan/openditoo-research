...
0. **Read `notes/OPENDITOO-HANDOFF-2026-09-10-W9.md` first** — the current handoff. W8 is closed PASS and
   ...
   W10A (Studio app, deadline scheduler, robust sender) is done offline and `OPENDITOO-WEBCAM-W10-001` is
   prepared, grant-ready and **unauthorized**. The W9 handoff lists the remaining lower-priority items.
...
| W10 webcam productization | **W10A done offline** (`OpenDitoo.Webcam.Studio`: preview with camera/mode, ROI/zoom, source + exact 16x16 matrix; absolute-deadline scheduler; duplicate/unchanged suppression with heartbeat; clean stop on Stop/close/unplug/stall; 17 C# checks + real-camera dry run). **W10B `OPENDITOO-WEBCAM-W10-001` prepared, grant-ready, UNAUTHORIZED** — one ≤60 s session, ≤500 frames. Standing (multi-session) webcam authority is W10C, a separate owner decision, not built. See `notes/OPENDITOO-W10-PLAN-2026-09-10.md` |
