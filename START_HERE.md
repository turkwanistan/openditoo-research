...
- **Handoff: `notes/OPENDITOO-HANDOFF-2026-09-11-POCKET-MOSS.md`** — Pocket Moss + reusable Pixel Animation Lab successor. Runtime 010 is implemented **offline only / unauthorized** on `feat/pocket-moss-pixel-lab`; exact grant `Grant OPENDITOO-PRODUCT-RUNTIME-010` is still required before any cutover or physical acceptance.
...
- **Successor branch:** `feat/pocket-moss-pixel-lab` contains Runtime 010 (Dashboard + Slots + Moss), the Pixel Animation Lab, modal navigation and hash-frozen Moss assets. The committed Runtime 010 template remains unauthorized; Runtime 009/main remains the live rollback baseline until an explicit grant/cutover.
