...
- **ACTIVE SOFTWARE RESEARCH: `notes/OPENDITOO-HANDOFF-2026-09-12-VOLATILE-RAM-API.md` + `notes/OPENDITOO-VOLATILE-RAM-API-PARSER-PLAN-2026-09-12.md`** — owner selected a no-hardware pivot: exhaust the stock Bluetooth/parser surface for a reversible **RAM-resident OpenDitoo API**. Start VRAM-0/VRAM-1 with a complete machine-readable 251-slot SPP/nested-mux atlas, then caller-controlled dataflow/memory-safety triage. Runtime 018 remains untouched. No live malformed/custom parser traffic without a new one-use manifest/grant. MassBoot/SPI measurement work is deferred; M2 remains authorized/unconsumed but must not be executed merely because authority exists.
...
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 361 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
