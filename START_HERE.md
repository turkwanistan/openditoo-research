...
- **ACTIVE SOFTWARE RESEARCH: `notes/OPENDITOO-HANDOFF-2026-09-12-VOLATILE-RAM-API.md` + `notes/OPENDITOO-VOLATILE-RAM-API-PARSER-PLAN-2026-09-12.md`** — Tier-2 has promoted a 4/4-branch RAM-only `CALLER_CONTROLLED_ADJACENT_HEAP_OVERWRITE` from stock `0x6c`: up to 2041 controlled source bytes reach 1032 bytes of physical capacity, for **up to 1009 controlled bytes beyond the allocation**. VRAM-3 is **closed positive**: the application heap is `0x00804000..0x0081cfff`, mapped executable under the stock ARMv5T MMU model; NX is not a blocker. Placement remains fail-closed: the tempting pristine-heap model would put the display backing at `0x00804470`, runtime50 at `0x00804b80`, and its `+0x34` callback at `0x00804bb4`, but framework-owned ordering between `Fwl_MallocInit` and `0x8dea` is not present as a recoverable in-image call/pointer edge, so those addresses are **conditional hypotheses, not promoted runtime facts**. The display root/backing has no audited stock app-heap teardown/recreate path, and the 0x80 callback-bearing plugin family has only two singleton constructor sites rather than an unbounded spray. `tools/ditoo_tier2_trigger.py` separately proves the runtime50 `+0x34` field is a real `BLX` control sink across 4/4 branches **and now proves VoiceTip's own period-1 microtask registration/dispatch 4/4**: the transformed `0x00218b43`/`0x00218b37` callback values decode exactly to the branch-local `0x1fb42`/`0x1fb36` worker, and resident timer code later `BLX`es the stored callback. The remaining trigger gate is narrower: prove an ordinary stock state transition that deterministically puts that periodic worker onto its normal completion/idle path to `0x1fa8a` after the overwrite. **No live manifest candidate exists.** Runtime 018 remains untouched; no malformed/custom live traffic without a new one-use manifest/grant.
...
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 379 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
