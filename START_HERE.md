...
**Current handoff (2026-09-10): physical button pagination is LIVE as Runtime 006.** Read `notes/OPENDITOO-HANDOFF-2026-09-10-BUTTONS.md` first — its "Next session" section is the entry point. Ditoo Left/Right page a wrap-around list (dashboard, spiral), a short lever pull runs the page's own action; Runtime 005 is the rollback. Next owner-selected objective: a high-FPS page on the existing `streaming_ack_clock` profile (one-use trial first). Research/plan notes (`…BUTTON-AVRCP-RESEARCH…`, `…PAGINATION-PLAN…`) are now history for BTN-0..6.
...
| BTN-9 high-FPS page | **next, not started**: streaming_ack_clock page via `frame_stream.stream_session`; branch/worktree + one-use trial first |
