...
- **Active recovery route: `notes/OPENDITOO-SOFTWARE-FIRST-RECOVERY-R0-2026-09-12.md`** — by owner preference, the prepared MassBoot M1 disassembly/photo session is deferred while software-only recovery avenues are exhausted. SFR-1 is **CLOSED** as `SPP_READBACK_NO_CALLER_CONTROLLED_CHAIN_WITHIN_ANALYZED_GRAPH`. SFR-3 is also **CLOSED**: the bounded official V2/V3 × flag42/60 × production/test matrix found no historical selector but `IsTest=true` exposed official test branches **flag42 v42017** and **flag60 API-v60016 / container-v60017**. SFR-4 immediately audited those new bytes: v42017 keeps all 251 top-level SPP handler targets identical to v42016 despite a material config/storage refactor, and the deeper no-caller-controlled-readback result survives; the flag60 test branch also passes the accepted SFR-1 invariants. Exact v42012 remains unrecovered. Next priority is **SFR-2 period-app/support-updater archaeology plus opportunistic provenance-trackable v42012 acquisition**. SD updater development stays parked until recovery/rollback is solved. M1 remains prepared/unauthorized; no hardware action is currently requested.
...
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 357 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
