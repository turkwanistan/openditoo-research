...
- **ACTIVE SOFTWARE RESEARCH: `notes/OPENDITOO-HANDOFF-2026-09-12-VOLATILE-RAM-API.md` + `notes/OPENDITOO-VOLATILE-RAM-API-PARSER-PLAN-2026-09-12.md`** — Tier-2 has promoted a 4/4-branch RAM-only `CALLER_CONTROLLED_ADJACENT_HEAP_OVERWRITE` from stock `0x6c`: up to 2041 controlled source bytes are copied into 1032 bytes of physical capacity, for **up to 1009 controlled bytes beyond the allocation**. VRAM-3 is now also **closed positive**: exact Ditoo evidence pins the application heap to `0x00804000..0x0081cfff` inside stock identity-mapped SRAM, with ARMv5T MMU + I/D caches enabled and no execute-never permission in the short-descriptor model; heap RAM is executable and **NX is not a blocker**. `tools/ditoo_tier2_placement.py` proves stock callback-bearing objects can be allocated/freed/recreated, but best-fit fragmentation plus zero-on-create means grooming is **not deterministic adjacency proof**. The sole promotion blocker is now `EXACT_DISPLAY_BLOCK_ADDRESS_OR_UNIQUE_ADJACENT_HOLE_MODEL` / deterministic control-flow victim placement. There is still **no live manifest candidate**. Runtime 018 remains untouched; no malformed/custom live traffic without a new one-use manifest/grant.
...
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 375 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
