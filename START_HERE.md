...
- **ACTIVE SOFTWARE RESEARCH: `notes/OPENDITOO-HANDOFF-2026-09-12-VOLATILE-RAM-API.md` + `notes/OPENDITOO-VOLATILE-RAM-API-PARSER-PLAN-2026-09-12.md`** — VRAM-0/1 and Tier-1 direct-SPP exhaustion remain closed, and **Tier-2 has promoted a real RAM-only primitive**. `tools/ditoo_tier2_display_surface.py` / `artifacts/analysis/volatile_ram_api_tier2_display.json` prove that the old "external media/codec" label was wrong: `0x6c` reaches the `divoom_light_word` resident display handoff, whose stage-1 copy at `0x008012a8` accepts a caller u16 length. The stock SPP assembler permits inner lengths through `0x800`, giving up to 2041 controlled source bytes, while the 0x708 backing allocation is rounded to 0x710 and exposes data at +0x308, leaving 0x408/1032 physical bytes. Result: **up to 1009 controlled bytes beyond the allocation** on all four preserved Plus branches, with no persistent-write dependency. This is `CALLER_CONTROLLED_ADJACENT_HEAP_OVERWRITE`, **not yet code execution**: the allocator is already fragmented/best-fit and no deterministic callback-bearing victim placement has been proven, so there is still **no live manifest candidate**. Immediate work is offline victim/control-sink placement plus VRAM-3 RAM-execution feasibility. Runtime 018 remains untouched; no malformed/custom live traffic without a new one-use manifest/grant.
...
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 368 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
