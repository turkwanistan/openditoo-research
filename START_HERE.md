...
- **Live product: Runtime 016** (`runtime_revision=11`, `host/product_runtime_v10.py`, policy `OPENDITOO-PRODUCT-RUNTIME-016`, main cutover `2c4177b`). Dashboard, Slots and Moss run in one `streaming_ack_clock` Host session. Ditoo target, Host `3faf520f…`, ButtonProbe, pacing, selector semantics, reconnect/reclaim and **webcam policy 006** are all unchanged. Runtime 015 is the exact rollback in `.openditoo-local/rollback-runtime-015` (`bash scripts/cutover_runtime_016.sh --rollback`). The earlier rollbacks for 014 and 013 are kept.
...
Bluetooth-media contention is localized and the receive-only successor lives on `feat/media-avrcp-input` in `.openditoo-local/worktrees/media-avrcp`. Raw AVRCP is complete (10/10 lever presses in saved HCI evidence); fixed-Playing SMTC is retained only as the media-ownership sink because its callbacks are lossy. Runtime 017 maps raw `0x4C` left, `0x4B` right and both `0x44|0x46` to lever. Live diagnostics proved all tested Ditoo actions share one ACL handle while a Tivoo negative control uses another, but BTVS started mid-connection reports zero Bluetooth addresses, so the committed exact-address filter is not yet viable. BTVS also requires elevation on this Windows host, so the final product must not assume an ordinary WSL child can launch it directly. **Runtime 017 is NOT live and has no grant; do not cut over.** Current focused successor tests are 17/17 PASS; the earlier interactive product suite remains 136/136 PASS. The immediate next experiment is the corrected low-level ACL attribution diagnostic after commit `c11ca8d`; run it from Administrator PowerShell with media paused/off, then use its result to finish dynamic Ditoo-handle learning. Read `notes/OPENDITOO-HANDOFF-2026-09-11-R017-MEDIA-AVRCP.md` first, then `notes/OPENDITOO-MEDIA-CONTENTION-RAW-AVRCP-2026-09-11.md`.
