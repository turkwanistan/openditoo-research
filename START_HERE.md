...
| W8 near-ceiling webcam rate | **003 pacing failure + 004 telemetry-validator failure consumed; 005 Windows prep PASS / grant-ready, unauthorized**: 50 ms client / 40 ms Host pacing survived 004, but an invalid cross-process clock comparison aborted after Host ACKed frame 9; fresh 005 keeps 50/40 pacing and fixes telemetry semantics only |
...
Fresh `OPENDITOO-WEBCAM-N980P-005` keeps the proven **50 ms client / 40 ms Host** pacing, same **201 / 603 / 211,854** hard limits, same camera/transform/Host protocol, and changes only telemetry validation: `hostFrameElapsedMs` must exist and remain within the reviewed **0..5000 ms ACK budget**, while the signed client-minus-Host elapsed residual is recorded observationally and never used as a cross-process ordering invariant. Offline regression includes a coarse-clock control that would have killed 004. Current suite is **256 tests PASS**. **Windows preparation for 005 is now PASS** and the candidate is **grant-ready and unauthorized**: the runner was rebuilt/staged/frozen, the historical 40 ms arrival-jitter failure reproduced, the 50 ms margin stayed clean, `ADAPTER_HOST_ELAPSED_CROSS_CLOCK_PASS` passed, transform/encoder parity and real read-only Host status identity passed, with `claim_created=false`, `host_session_io=false` and `device_io=false`. No 005 claim or ledger entry exists. Manifest `032dcc81...`, preparation evidence `55a717ca...`; see route §25. The only remaining gate is the exact operator grant `Grant OPENDITOO-WEBCAM-N980P-005`, after which only `scripts/run_webcam_w8_telemetry_rerun_windows.ps1` may execute, exactly once.
