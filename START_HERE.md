...
- **Handoff: `notes/OPENDITOO-HANDOFF-2026-09-11-POCKET-MOSS-V2.md`** — Runtime 010 is now live/working, but v1 Moss failed physical readability (brown blob). Runtime 011 is the offline/unauthorized art-only v2 successor on `feat/pocket-moss-v2-art-r2`; exact grant `Grant OPENDITOO-PRODUCT-RUNTIME-011` is required before v2 physical acceptance.
- **Prior Pocket Moss handoff:** `notes/OPENDITOO-HANDOFF-2026-09-11-POCKET-MOSS.md` documents the v1 implementation and Runtime 010 cutover path.
- **Prior accepted handoff:** `notes/OPENDITOO-HANDOFF-2026-09-11-HF4.md` for the live Runtime 009 baseline and HF-4 evidence.

- **Live product: Runtime 010** (Pocket Moss v1): one `streaming_ack_clock` Host session carries Dashboard + Slots + Moss; generic modal Moss selector/actions are working physically; Host `3faf520f…`; **webcam policy 006** unchanged. Code `host/product_runtime_v4.py`, local standing policy `OPENDITOO-PRODUCT-RUNTIME-010` (`runtime_revision=5`). v1 Moss visual acceptance failed only on recognizability; Runtime 011 is the prepared art successor. Exact-tree rollback for the next cutover is Runtime 010.
...
- **Successor branch:** `feat/pocket-moss-v2-art-r2` contains Runtime 011, the complete Moss v2 art vocabulary, readability regressions and guarded 011 cutover. The committed 011 template remains unauthorized; live Runtime 010/main is the rollback baseline until a fresh 011 grant/cutover.
