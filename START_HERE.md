...
- **Active recovery route: `notes/OPENDITOO-SOFTWARE-FIRST-RECOVERY-R0-2026-09-12.md`** — by owner preference, the prepared MassBoot M1 disassembly/photo session is deferred while software-only recovery avenues are exhausted. SFR-1 is now **CLOSED** as `SPP_READBACK_NO_CALLER_CONTROLLED_CHAIN_WITHIN_ANALYZED_GRAPH`: a fail-closed schema-v3 analyzer proves the five analyzed raw-SPI-read caller families are typed/internal config, resource or screen-save storage operations; even the one-byte raw-address parser receives only firmware-managed config/resource addresses on all analyzed SPP provenance paths across both preserved Plus branches. `0x93..0x96` remain default/error, `0x98/0x99` update ingress, and EXTERN is narrow. Next priority is **SFR-2 period-app/updater archaeology + SFR-3 bounded official API matrix**, then lineage expansion if new firmware appears. SD updater development stays parked until recovery/rollback is solved. M1 remains prepared/unauthorized; no hardware action is currently requested.
...
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by the software-first recovery route above (with MassBoot R2 preserved as the deferred physical fallback). R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 353 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware/recovery-R&D failures.
