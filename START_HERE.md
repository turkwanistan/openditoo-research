...
- **Firmware/input-takeover handoff: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — current research checkpoint. SD updater path is physically proven on the exact v42012 unit; retail-USB MassBoot enumeration stayed negative across two bounded trials; M-at-boot factory mode is confirmed (`42 / 012`). **Offline R&D progress (2026-09-12):** FIRM-R0 — full keypad event graph byte-verified; the true common fan-in is the **category-`0x82` queue post** (`0xc6d7c`), not the emitter alone (auto-repeat/finalize bypass it). UPDATE-R0 — SD container fully modelled (`[u32 ver][12 "DIVOOMUPDATE"][u32 additive checksum]`; gates marker→version→checksum→writer `0x3a824`) with an offline validator. PATCH-R0 — non-deployable fail-open hook prototype (fail-closed, verifies original bytes, emits only non-installable research images proven rejected by the updater). FACTORY-R0 — product-test state machine mapped; key-test stage read-only, SPI-flash stage conservatively unsafe. TELEMETRY-R0 — reuse the stock SYS SPP device→host path. RECOVERY-R0 — **still the binding blocker**: exact v42012 unrecovered, documented v42010/v60010 CDN objects 404, no SPP readback. Offline verifier **342 tests PASS**. Tools: `tools/keypad_pipeline_report.py`, `tools/ditoo_update_container.py`, `tools/openditoo_patch_prototype.py`; artifact `artifacts/analysis/keypad_pipeline.json`. Read this handoff first for firmware/recovery work, then the dedicated plan `notes/OPENDITOO-FIRMWARE-INPUT-TAKEOVER-PLAN-2026-09-12.md`. **No checksum-valid custom firmware or fresh physical experiment is authorized at this handoff.**
