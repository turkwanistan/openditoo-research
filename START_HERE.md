...
- **Active firmware/recovery handoff: `notes/OPENDITOO-HANDOFF-2026-09-12-MASSBOOT-R2.md`** — MassBoot has been reopened as the recovery priority after distinguishing the already-tested firmware soft handoff (MB-S) from the AK1052D reset-time GPIO6 hardware bootstrap (MB-H). The execution plan is `notes/OPENDITOO-MASSBOOT-R2-EXECUTION-PLAN-2026-09-12.md`; research rationale is `notes/OPENDITOO-MASSBOOT-RENEWED-STRATEGY-2026-09-12.md`. R2-0 offline preparation is complete; immediate route is one exact-unit **photos-only** M1 session under prepared-but-unauthorized manifest `experiments/OPENDITOO-MASSBOOT-M1-PHOTOS-001.json`. No fresh physical action is authorized by this handoff.
- **Firmware R0/R1 technical record: `notes/OPENDITOO-HANDOFF-2026-09-12-FIRMWARE-R0.md`** — detailed historical record, superseded for current routing by MassBoot R2 above. R1 closed the category-`0x82` producer/consumer seams, updater writer topology, fail-closed non-installable patch prototype, and SYS-SPP telemetry choice. It also proved factory initialization is **destructive/persistent-write** (4 KiB erase/write/read), so factory mode is closed. Exact v42012 remains unrecovered. Current constrained-sandbox verifier baseline is 345 tests with only the two known unrelated missing-Pillow W9B errors and zero firmware-R&D failures.
