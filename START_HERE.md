...
- **Handoff: `notes/OPENDITOO-HANDOFF-2026-09-11-POCKET-MOSS-V3.md`** — Runtime 011 is live and physically successful enough to preserve: v2 fixed the v1 “brown blob” recognition failure. Owner feedback after live v2: “much better” / “pretty great already,” but the idle pose is still a little derpy/flat and the animations need more personality. Runtime 012 is the offline/unauthorized v3 polish successor on `feat/pocket-moss-v3-polish`.
- **Live product: Runtime 011** (`runtime_revision=6`, policy `OPENDITOO-PRODUCT-RUNTIME-011`): Dashboard + Slots + Moss remain in one `streaming_ack_clock` Host session; exact Ditoo target, Host `3faf520f…`, ButtonProbe, pacing, selector semantics, reconnect/reclaim behavior and **webcam policy 006** are unchanged. Runtime 011 is the exact rollback baseline for 012.
- **Runtime 012 candidate:** v3 retains the successful floppy-ear / cream-muzzle dog identity, adds an amber depth/shadow plane and cleaner consistent body anatomy, and expands authored motion: Dance 8 unique frames, Pet 6, Kisses 5. The committed 012 template is unauthorized and requires exact grant `Grant OPENDITOO-PRODUCT-RUNTIME-012`.
- **Offline gate:** Runtime 012 current-template checks PASS; v2 historical fixtures remain explicit; Pixel Lab warnings are zero; canonical interactive successor gate PASS **100/100**. Cutover helper: `scripts/cutover_runtime_012.sh`; rollback restores Runtime 011 exactly.
...
- **Environment:** live cutovers run from normal local WSL with `/mnt/c` + `powershell.exe`, not WSL_MCP. Worktree metadata should be mount-independent/relative before handoff.
